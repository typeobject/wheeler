...
classical class Verifier {
  ...
  private long readUnsigned(borrow byteview artifact, long offset, long width) {
    long value = 0;
    long multiplier = 1;
    long cursor = 0;
  ...
  private long directoryField(borrow byteview artifact, long section, long field, long width) {
    return readUnsigned(artifact, 40 + section * 32 + field, width);
  }

  private boolean magicValid(borrow byteview artifact) {
    if (artifact[0] == 87) {
      if (artifact[1] == 72) {
        if (artifact[2] == 69) {
          if (artifact[3] == 69) {
  ...
  private long verifyDirectory(borrow byteview artifact, long fileLength, long sectionCount) {
    long section = 0;
  ...
  private long verifyPayloads(borrow byteview artifact, long sectionCount) {
  ...
  public long verifyArtifact(borrow byteview artifact, long fileLength) {
    if (fileLength < 320) {
      return 0;
    }
