        ...
        if (region < localCount) {
          if (length < localCount) {
            if (localHasType(artifact, activeTypes, first, allocationType)) {
              if (
                ownerOrBorrow(localType(artifact, activeTypes, region), TYPE_REGION)
              ) {
      ...
      if (first < localCount) {
        if (setIndex < localCount) {
          if (setValue < localCount) {
            if (
              ownerOrBorrow(localType(artifact, activeTypes, first), setExpectedType)
            ) {
      ...
      if (first < localCount) {
        if (scalarBuffer < localCount) {
          if (scalarIndex < localCount) {
            if (localHasType(artifact, activeTypes, first, TYPE_SIGNED)) {
              if (utf8SequenceType(localType(artifact, activeTypes, scalarBuffer))) {
                if (localHasType(artifact, activeTypes, scalarIndex, TYPE_SIGNED)) {
                  return 1;
                }
              }
            }
          ...
          if (widthIndex < localCount) {
            if (localHasType(artifact, activeTypes, first, TYPE_SIGNED)) {
              if (utf8SequenceType(localType(artifact, activeTypes, widthBuffer))) {
                if (localHasType(artifact, activeTypes, widthIndex, TYPE_SIGNED)) {
                  return 1;
                }
              }
            }
      ...
      if (first < localCount) {
        if (mapKey < localCount) {
          if (mapValue < localCount) {
            if (
              ownerOrBorrow(localType(artifact, activeTypes, first), TYPE_LONG_MAP)
            ) {
      ...
      if (first < localCount) {
        if (getMap < localCount) {
          if (getMapKey < localCount) {
            if (localHasType(artifact, activeTypes, first, TYPE_SIGNED)) {
              if (
                ownerOrBorrow(localType(artifact, activeTypes, getMap), TYPE_LONG_MAP)
              ) {
      ...
      if (first < localCount) {
        if (hasMap < localCount) {
          if (hasMapKey < localCount) {
            if (localHasType(artifact, activeTypes, first, TYPE_BOOLEAN)) {
              if (
                ownerOrBorrow(localType(artifact, activeTypes, hasMap), TYPE_LONG_MAP)
              ) {
      ...
      if (first < localCount) {
        if (mapBorrowSource < localCount) {
          if (localHasType(artifact, activeTypes, first, TYPE_LONG_MAP_BORROW)) {
            if (
              ownerOrBorrow(localType(artifact, activeTypes, mapBorrowSource), TYPE_LONG_MAP)
            ) {
              return 1;
            }
          }
    ...
    if (opcode == OPCODE_BUFFER_BORROW) {
      ...
      if (first < localCount) {
        if (bufferBorrowSource < localCount) {
          ...
          long bufferBorrowSourceType = localType(artifact, activeTypes, bufferBorrowSource);
      ...
      if (first < localCount) {
        if (regionBorrowSource < localCount) {
          if (localHasType(artifact, activeTypes, first, TYPE_REGION_BORROW)) {
            if (
              ownerOrBorrow(localType(artifact, activeTypes, regionBorrowSource), TYPE_REGION)
            ) {
              return 1;
            }
          }
