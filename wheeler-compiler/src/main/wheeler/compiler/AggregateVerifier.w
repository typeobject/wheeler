...
classical class AggregateVerifier {
  ...
  private long recordFieldType(borrow byteview artifact, long descriptor, long field) {
  ...
  private long variantCaseDescriptor(borrow byteview artifact, long descriptor, long tag) {
  ...
  private long variantFieldType(borrow byteview artifact, long caseDescriptor, long field) {
  ...
  private long variantOperandsValid(
    ...
    if (opcode == OPCODE_VARIANT_NEW) {
      ...
      if (destination < localCount) {
        if (
          differs(localType(artifact, activeTypes, destination), TYPE_VARIANT + typeId)
        ) {
          return 0;
        }
      } else {
                ...
                if (0 < arrayInfo) {
                  if (0 < sliceInfo) {
                    if (
                      ...
                      if (localType(artifact, activeTypes, startLocal) == TYPE_SIGNED) {
                        if (
