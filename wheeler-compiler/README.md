...
`wheeler test wheeler-compiler` discovers and executes the checked-in `nativecompilerspinetests` target through the Wheeler runtime. Seven targets compile twenty-three distinct physical compiler modules through separate bounded source graphs and publish twenty-three independent cases. The spine includes callable `BooleanTokens.w`. The syntax partition covers storage, loops, helper ABI, borrowed intrinsics, keywords, and source scalars. The call-syntax partition executes `VoidCallSourceKinds.w` beside assignment-call identities and loop-body layouts. The type-kind partition executes the function-only `TypeKinds.w` decoder over the physical type-code table. The call-arity partition executes `AssignmentCallArities.w` over the physical call-assignment identity table. The call-width partition executes the code, instruction, and local width decoders through their shared nested arity helper. The call-operand partition executes the four-argument packed-source decoder with its private helper and imported arity owner. It resolves `ENCODING_WIDTH_U16` from production source, checks the value, carries it through checked addition, subtraction, multiplication, division, remainder, bitwise AND, bitwise XOR, and signed ordering, and checks every result natively. Java does not discover or execute the case. The complete compiler graph is not yet inside this package suite.
