...
- `compiler/closure/layouts/source/instructions/SourceAggregateOperations.w` indexes bounded aggregate constructors and projections without publishing partial ranges. `compiler/closure/layouts/LocalNominalReferences.w` indexes local nominal uses outside declaration bodies, and `LocalNominalCarriers.w` rewrites them to compact signed carriers with exact transformed coordinates. `PrimitiveFrontendProducts.w` publishes named values and separate source and splice statement ordinals. `AggregateFrontendBindings.w` joins those frontend products to exact locals and splice coordinates. `AggregateConstructorTargets.w` resolves local constructor coordinates. `AggregateProjectionTargets.w` resolves fields and indexes inside explicit owner windows. `AggregateResolvedOperands.w` joins source-ordered local, descriptor, case, and member products. `ResolvedAggregateOperations.w` admits exact opcodes and operands according to each source kind. `AggregateInstructionComposition.w` merges primitive and aggregate code at validated source-order splice points. Supplemental code archival and counted artifact selectors carry that order into final linked code.
