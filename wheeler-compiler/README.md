...
`wheeler test wheeler-compiler` discovers and executes every checked-in native compiler target through the Wheeler runtime. Seventy-five targets compile ninety-seven distinct physical compiler modules through separate bounded source graphs and fill the 255-case native report profile. The spine includes callable `BooleanTokens.w`. The syntax partition covers storage, loops, helper ABI, borrowed intrinsics, keywords, and source scalars. The call-syntax partition executes `VoidCallSourceKinds.w` beside assignment-call identities and loop-body layouts. The type-kind partition executes the function-only `TypeKinds.w` decoder over the physical type-code table. The call-arity partition executes `AssignmentCallArities.w` over the physical call-assignment identity table. The call-width partition executes the code, instruction, and local width decoders through their shared nested arity helper. The assignment-call operand partition executes its four-argument packed-source decoder with a private helper and imported arity owner. The void-call operand partition executes two complete three-member owners and decodes one trailing source. The void-call width partition checks both physical width functions through the shared shape-query owner. The source-width partition checks direct named and nested resolved local-layout paths. The source-form partition checks membership and both directions of the source identity and arity map. The resolved-kind partition calls all three physical shape queries directly. The assignment-call column partition checks both upper-bound identity maps. The assignment-call kind partition executes both classifiers, identity construction, and inverse target projection through one shared declaration graph. The helper-signature partition retains the complete nine-member owner and executes every public query at a meaningful admitted bound. The wide-return partition packs distinct prior locals into both physical operands and executes all seven inverse source queries. The instruction-form partition reaches the private storage classifier through the canonical opcode tables. The opcode-kind partition executes every family classifier, including the nested local-math path. The identifier partition proves that one imported helper keeps its physical module-qualified name. The signed helper-result partition reaches every direct, local, arithmetic, forwarding, borrowed, UTF-8, map, and pair family through eleven cases and one six-source graph. The reversible-token partition retains a generated inverse and qualified proof, then executes the result-slot call under native coverage. The resolved-return partition executes aggregate membership, signed membership, and source decoding at exact column boundaries. The resolved equality, inequality, less-than, local-literal comparison, assertion, early-result, assignment, copy, and signed-operation partitions import the complete ninety-two-constant statement-opcode owner and execute one hundred thirty-nine decoder and classifier queries. The resolved-loop partitions execute all four form decoders, both packed operand decoders, and the complete 6,144-opcode classifier from physical loop-kind and statement-opcode constants. The spine resolves `ENCODING_WIDTH_U16` from production source, checks the value, carries it through checked addition, subtraction, multiplication, division, remainder, bitwise AND, bitwise XOR, and signed ordering, and checks every result natively. Java does not discover or execute the case. The complete compiler graph is not yet inside this package suite. CI assigns its complete case identities to sixteen disjoint shards before compilation. Each selected artifact executes once with fresh storage.

Separate closure evidence retains 108 physical source products. Direct `AssignmentCallSyntax.w`, `WideLocalCalls.w`, `EarlyUtf8CallForms.w`, and the focused result owners require no dependency-source projection. The resulting executable subset contains 280 functions and 10,819 instructions.
...
### Retained call profile

The source-product route admits zero through eight ordered identifier arguments
in ordinary root and loop calls, including qualified imports. Signed and Boolean values,
UTF-8 and byte-view loans, and mutable byte and word loans keep exact types.
`compiler/closure/layouts/source/calls/SourceCallArgumentLayouts.w` owns the
256-call profile and its two 4,096-word argument tables. Each column holds 2,048
arguments. Binding, final-return widths, typed layout, code planning, loop
emission, and imported signature stubs use the same arity limit.

A ninth argument or mismatched type rejects before publication. Argument, type,
frame, token, code, and artifact capacities remain separate. Imported-target
buffer checks live in `StructuredSourceTargets.w`. The orchestrator calls them
with eight arguments and stays within its unchanged native token arena. This
profile does not widen the bounded helper compiler described below. Generated
inverses still reject argument-bearing calls. Widening arity does not implement
their transfer and cleanup inverses.
...
`ScalarHelperPrograms.w` constructs the IR. `HelperCallSites.w` supplies one call-row
collector to helper and entry bodies. A table of one through twenty-three helpers may
precede an entry. Imported owner metadata selects canonical library strings even for one helper. Every entry call resolves to an exact table row, code generation emits
the entry after the helper bodies, and the entry retains its module-qualified `main`
identity. That path accepts zero or one general helper and one through twenty-three
public, private, or unqualified zero- through sixteen-parameter scalar helpers. A one-member array library no longer needs a dummy
...
products remove the final physical signature-stub route. `AssignmentCallSyntax.w` and
`WideLocalCalls.w` now cross the same direct boundary. Their bounded recursive scans keep
intrinsic reads and call arguments explicit, while unique sorted verifier stubs remain outside the
retained closure. Imported relocation validation keeps its
...
active target, parameter, and dependency rows. Qualifier and referenced-target products likewise
publish only rows selected by imported targets and source calls. Source value, callable-local, and
statement-local products publish only their measured rows. Resolved body, nested-control, and loop
frame-width products do the same. Source statement, condition, and loop products likewise publish
only measured rows. Flat callable statements, balanced block trees, resolved loops, physical bodies,
and loop-local types do the same. Source-call layouts update only active calls and their owning
statement widths. Call emission likewise publishes only active relocation, identity, type, width,
and code products. Callable composition publishes only active callable and local-type rows. Callable
and source-product coordinates likewise publish only planned rows. Call-instruction windows and
callable-return products follow their active counts. Reversible result types, inverse windows,
relocations, and proof names publish measured rows and bytes. Decoded compiled functions and
instructions publish only canonical artifact counts. Source-artifact bytes and archive-source index
rows publish exact measured extents. Ordinary structured direction selection preserves that exact
artifact boundary. Parsed, descriptor-compatible, and counted aggregate layouts likewise move only
measured aggregate, case, member, and string rows. Aggregate operations, arguments, and resolved
operands also publish exact product counts. Constructor and projection targets retain only active
operation rows. Aggregate frontend locals, arguments, and primitive placeholder placements follow
active operation and argument counts. Primitive placeholder filtering carries only active function,
instruction, and placement rows. Aggregate instruction composition stages and publishes measured
function, instruction, and selector products. Imported and local nominal projections move only
validated carrier rows. The aggregate body transaction transfers every product through its owning
plan count. Nested aggregate expression values stage and publish only active value columns.
Aggregate source owners, indexed owners, slice descriptors, and structural values publish through
operation and value counts. Ownership projections and aggregate operand relocations publish exact
event and relocation extents. Source-planned and instruction-decoded ownership coordinates use the
...
slice sentinel staging follows active operation counts. Invalid aggregate-operation scans terminate
without consuming the loop bound. `StatementKinds.w` owns 139 unresolved statement identities. `LoopKinds.w` owns six loop-form
