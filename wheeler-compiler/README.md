...
`wheeler test wheeler-compiler` discovers and executes the checked-in `nativecompilerspinetests` target through the Wheeler runtime. Twelve targets compile twenty-eight distinct physical compiler modules through separate bounded source graphs and publish thirty-four independent cases. The spine includes callable `BooleanTokens.w`. The syntax partition covers storage, loops, helper ABI, borrowed intrinsics, keywords, and source scalars. The call-syntax partition executes `VoidCallSourceKinds.w` beside assignment-call identities and loop-body layouts. The type-kind partition executes the function-only `TypeKinds.w` decoder over the physical type-code table. The call-arity partition executes `AssignmentCallArities.w` over the physical call-assignment identity table. The call-width partition executes the code, instruction, and local width decoders through their shared nested arity helper. The assignment-call operand partition executes its four-argument packed-source decoder with a private helper and imported arity owner. The void-call operand partition executes two complete three-member owners and decodes one trailing source. The void-call width partition checks both physical width functions through the shared shape-query owner. The source-width partition checks direct named and nested resolved local-layout paths. The source-form partition checks membership and both directions of the source identity and arity map. The resolved-kind partition calls all three physical shape queries directly. It resolves `ENCODING_WIDTH_U16` from production source, checks the value, carries it through checked addition, subtraction, multiplication, division, remainder, bitwise AND, bitwise XOR, and signed ordering, and checks every result natively. Java does not discover or execute the case. The complete compiler graph is not yet inside this package suite.
