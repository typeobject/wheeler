...
the five-module direct star, chain, four-leaf fork, three-leaf fork beside a direct import, one chain edge beside three direct imports, a two-leaf fork beside two direct imports, two independent chains beside a direct import, a three-module chain beside two direct imports, a four-module chain beside a direct import, a nested two-leaf fork beside a direct import, two nested fork levels, or a shared diamond with a side leaf. `compileMinimalWithSixConstantImports` links the six-module direct star, full chain, five-leaf fork, one three-leaf fork beside two direct imports, one nested two-leaf fork beside two direct imports, one uneven two-branch tree beside two direct imports, one fork beside one chain and one direct import, three independent chains, one three-module chain beside one two-module chain and one direct import, one chain edge beside four direct imports, one two-leaf fork beside three direct imports, one three-module chain beside three direct imports, one four-module chain beside two direct imports, and two independent chains beside two direct imports. `compileMinimalWithSevenConstantImports` links the twenty-eight admitted seven-module forms, including every five-node rooted tree beside two direct imports and three shared-DAG forms. The exhaustive five- and six-module fixtures and forward/reverse seven-module rotations make source-frame order nonauthoritative. Every planner validates exact edges, roots, root-import rank, reachability, visibility, sharing, and canonical role order before source rewriting. `graphs/plans/PlanSources.w` binds table initialization, replacement, and copying to the validated node count. Two- through four-module drivers select canonical roles only from that table. Their planned-loan selector was deleted. `graphs/plans/ConstantExecutor.w` walks leaf-first edges and dependency-aware root-import ranks for every validated two- through seven-module constant graph. Forests, redundant direct edges, the three-root shared leaf, and both admitted shared diamonds use this path. Repeated private leaves disappear only after exact declaration comparison. Five-module fallback execution retains only direct helper owners and one constant-fed helper chain beside direct constants. Ten five-module topology executors were deleted. Pure constants cannot reach this fallback. This path has no topology identity or frame-order probe. `graphs/plans/SourceTable.w` owns seven fixed 32,768-byte physical or linked slots and their active lengths in one table. Initialization validates every active source before writing. Replacement validates the complete frozen source, clears the old tail, and leaves the old slot intact on rejection. The synthetic root has a separate fixed buffer, so seven imports do not turn the table into an eight-node loophole. Header dependency facts retain the candidate's import rank. Checked matrix accessors expose ordered nodes, edges, root ranks, privacy, and sharing without topology dispatch. Small direct plans use root-header rank instead of frame arrival. The two-module executor handles direct, chain, and redundant-edge plans without a topology identity. The three-module executor handles direct, chain, fork, and chain-plus-direct plans the same way. The four-module executor consumes all ten admitted forests and the shared diamond from structural plan facts rather than topology identities. `graphs/SmallPlanSources.w` and `graphs/FourPlanSources.w` own canonical role selection outside the drivers. `graphs/TwoRedundant.w` handles the two-module chain whose leaf and dependent are both direct root imports. `graphs/shared/ThreeDirectLeaf.w` handles one public constant leaf shared by two direct constant dependents. Both drop exact private copies while retaining the public leaf declaration. The two-module path may also insert dependent helpers after declarations. A leaf export otherwise becomes private inside its dependent, so spelling the leaf name loudly grants nothing.
