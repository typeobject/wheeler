...
owns separate source and resolved `bufferLength`, `utf8Scalar`, `utf8Width`, indexed-buffer, and mutable-buffer statement identities. `VoidCallKinds.w` owns the zero-, one-, and two-argument ordinary void-call forms. Focused frontend syntax, typed resolution, call-argument, and backend codegen owners validate, resolve, and emit the canonical forms. `Tokens.w` now sticks to
