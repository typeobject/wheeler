...
contain bounded equality or less-than guards, computed signed-local preludes, and up to sixty-four same-module Boolean calls with typed
early returns. Sixty-four call sites fill the 256-local window without crossing the 512-instruction ceiling; call sixty-five fails closed. A final Boolean return may forward one zero-, one-, or two-argument helper call. A signed less-than guard may
...
One through seven direct helper edges may jointly contribute one through twenty-two helpers while the root owns the remainder of the twenty-three-helper table. `backend/HelperOwners.w` owns one validated seven-slot owner table, so `compiler/Core.w` has one owner-aware entry instead of four arity-shaped copies. `frontend/modules/MultipleImportedHelpers.w` orders up to four owner groups by validated root imports rather than frame arrival. Eight-plus-seven-plus-seven dependency helpers match stage 0 in all six source orders, and a six-plus-six-plus-five-plus-five split matches across eight four-owner rotations, and `frontend/modules/WideImportedHelpers.w` checks 5+5+4+4+4 across ten five-owner rotations, while `SixHelperOrder.w` and `SixImportedHelpers.w` check 4+4+4+4+3+3 across twelve six-owner rotations, while the corresponding seven-owner pair checks 4+3+3+3+3+3+3 across fourteen; a twenty-third dependency helper, eighth executable owner, or twenty-fourth total helper fails before publication. One three-module chain may first resolve a constant owner into an executable dependency. Both paths preserve function names and private visibility. Other executable imported members,
