...
`wheeler test wheeler-compiler` discovers and executes the checked-in `nativecompilerspinetests` target through the Wheeler runtime. The target compiles seven physical compiler constant modules and one test root. It resolves `ENCODING_WIDTH_U16` from production source, checks the value, carries it through checked addition, subtraction, multiplication, division, and remainder, and checks every result natively. Java does not discover or execute the case. The complete compiler graph is not yet inside this package suite.
