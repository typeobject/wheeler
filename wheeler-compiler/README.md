...
entryless library. `frontend/helpers/ScalarHelperTables.w` owns bounded helper lookup, duplicate checks, and call resolution. `ScalarHelperLibraries.w` parses members, and `ScalarHelperPrograms.w` assembles the complete table. That path accepts zero or one general helper, or two through ten explicitly
...
return its parameter minus or modulo one scalar. An eleventh helper fails
...
spelling the leaf name loudly. One bounded direct edge may give a dependency one through nine helpers while the root owns the remainder of the ten-helper table.
