...
identities. `ResolvedStatements.w` owns eighty resolved columns. `BorrowedIntrinsicReturns.w`
owns the separate source and resolved `bufferLength` statement identities. `Structure.w` validates
the closed syntax, and the backend intrinsic owner emits the canonical instruction shapes. `Tokens.w` now sticks to
