...
- `compiler/closure/symbols` owns packed direct-import values and package-bound symbol identities without enlarging the source indexer past its native token window. Resolved scalar module identities bind the package, source, module name, direct dependency products, symbols, resolution bits, and values in leaf-first order. External edges fail closed until locked package product identities exist. The bounded constant evaluator consumes those products for unqualified and qualified expressions. `compiler/closure/products` publishes counted callable signatures and complete body ranges, rewrites only an executable root against completed scalar products, and compiles arbitrary counted scalar closures without `BoundedGraphPlan` or dependency source. Callable ranges remain archive evidence until typed signatures and compiled body identities publish.
