...
- `compiler/closure/layouts/source/instructions/SourceAggregateOperations.w` indexes bounded aggregate constructors and projections without publishing partial ranges. `AggregateConstructorTargets.w` resolves local constructor coordinates. `AggregateProjectionTargets.w` resolves fields and indexes inside explicit owner windows. `ResolvedAggregateOperations.w` admits exact opcodes and operands according to each source kind.
