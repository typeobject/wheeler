...
- `compiler/Graphs.w` coordinates one- through three-module plans and bounded helper fallbacks.
- `compiler/GraphFour.w`, `GraphFive.w`, and `GraphSix.w` coordinate their counted frames without topology dispatch.
...
- `compiler/graphs/plans/ConstantExecutor.w` executes every validated constant plan. Dense DAGs, redundant direct edges, shared leaves, forests, chains, and forks are graph data, not executor identities.
- `compiler/graphs/Sources.w` retains one counted selector for the constant-fed helper fallback. Invalid indexes trap instead of selecting a padded loan.
- `compiler/graphs/direct/MixedTwo.w`, `MixedThree.w`, and `MixedFour.w` preserve root-import order for direct helper and constant owners. `compiler/graphs/five/FiveMixed.w` retains the one constant-fed helper chain. `frontend/modules/CanonicalHelperLinking.w` keeps constants ahead of functions.
...
before publication. The checked-in `compiler/backend/calls/CallArguments.w`, `compiler/backend/EncodingWidths.w`, imported-function `compiler/frontend/modules/HelperSourceOrder.w`, imported-function `compiler/frontend/order/HelperSourceNetwork.w`, imported-constant `compiler/frontend/intrinsics/BorrowedIntrinsicShapes.w`, `compiler/ir/Opcodes.w`,
