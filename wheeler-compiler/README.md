...
- `compiler/closure/symbols` owns packed direct-import values and package-bound symbol identities without enlarging the source indexer past its native token window. The bounded constant evaluator consumes those products for unqualified and qualified expressions. `compiler/closure/products` rewrites only an executable root against completed scalar products and compiles arbitrary counted scalar closures without `BoundedGraphPlan` or dependency source.
