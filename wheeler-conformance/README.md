...
- `testing/` publishes runtime-owned identities, summaries, reports, outcomes, and metadata.
  ...
  A one-case descriptor over one root and up to seven local imported sources may request native entry compilation with zero artifact length and `<target>::entry`. One parameterless root test over one root and up to seven local imports may instead use zero artifact length and `<target>::<declaration>`. The native runner discovers, lowers, compiles, verifies, and executes it without a Java-supplied artifact.
