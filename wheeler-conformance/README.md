...
- `testing/` publishes runtime-owned case, execution, and coverage identities, shard assignments, canonical summaries, report identities for up to 64 cases, bounded artifact outcomes, verified artifact metadata, composed artifact execution identities, and a bounded zero-to-64-case test publisher over one natively hashed canonical package manifest with native package, deployable-target, root-module, and test-selection checks, one root-bound dependency-free lock, runtime-owned descriptor validation, one manifest-selected, natively validated strict-UTF-8 unique-module, local-import-resolved target-source plan and identity, strictly ordered complete case names, shard selection, execution, diagnostics, and reduction.
