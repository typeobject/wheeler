...
- `testing/` publishes runtime-owned identities, summaries, reports, outcomes, and metadata.
  ...
  A one-case descriptor over one root and up to four local imported sources may request native compilation with zero artifact length. The resulting artifact enters the same profile-2 report path as transported bytes.
