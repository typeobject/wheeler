...
- `testing/` publishes runtime-owned identities, summaries, reports, outcomes, and metadata.
  ...
  For transported artifacts, the runner discovers up to 64 parameterless root `test void` declarations with the native lexer and binds each `<target>::<declaration>`. The resulting artifact enters the same profile-2 report path.
