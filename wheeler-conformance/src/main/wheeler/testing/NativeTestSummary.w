...
classical class NativeTestSummary {
  ...
  /// Publishes selected, passed, failed, and successful fields after canonical reduction.
  ///
  /// - Effects: Publishes only after sorting and validating every case outcome.
  entry void main(borrow byteview input, borrow mut bytes output) {
    ...
    setOutputLength(output, SUMMARY_BYTES);
    drop(counts);
    drop(right);
    drop(left);
    drop(staging);
