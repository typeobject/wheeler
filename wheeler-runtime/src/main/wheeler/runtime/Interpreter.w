...
import wheeler.runtime.interpreter_layout;
...
classical class Interpreter {
  ...
  public ExecutionResult executeVerifiedArtifact(
    ...
    borrow mut words returnDirections,
    borrow mut words aggregateTypes,
    borrow mut words aggregateTags,
    borrow mut words aggregateStarts,
    borrow mut words aggregateCounts,
    borrow mut words aggregateFields,
    borrow mut words storageKinds,
    borrow mut words storageStarts,
    borrow mut words storageLengths,
    borrow mut words storageSizes,
    borrow mut words storageOwners,
    ...
    long steps = 0;
    long clear = 0;
    while (clear < readUnsigned(artifact, entryDescriptor + 32, 4)) limit INTERPRETER_LOCAL_WIDTH {
      set(locals, clear, 0);
      clear += 1;
    }
    ...
    while (steps < stepLimit) limit MAX_INTERPRETED_STEPS {
      ...
      setByte(traceControl, steps, direction * 4);
      ...
      if (opcode == OPCODE_RETURN_RESULT_SLOT) {
        if (0 < depth) {
          ...
          direction = returnDirections[depth];
      ...
      if (opcode == OPCODE_RETURN) {
        ...
        if (opcode == OPCODE_RETURN_VALUE) {
          ...
          if (opcode == OPCODE_ADD_CONST) {
            ...
            set(globals, addGlobal, globals[addGlobal] + readSigned(artifact, cursor + 16));
          }
          ...
          if (opcode == OPCODE_CALL) {
            ...
            if (callTarget < functionCount) {
              ...
              set(returnDirections, depth, direction);
              depth += 1;
              function = callTarget;
              instruction = 0;
              direction = 0;
          ...
          if (opcode == OPCODE_UNCALL) {
            ...
            if (uncallTarget < functionCount) {
              ...
              long clearUncall = 0;
              while (clearUncall < readUnsigned(artifact, uncallDescriptor
                + 32, 4)) limit INTERPRETER_LOCAL_WIDTH {
          ...
          if (opcode == OPCODE_CALL_VALUE) {
            ...
            if (valueTarget < functionCount) {
              ...
              long valueTypes = parameterTypes(
                artifact,
                functionsOffset,
                functionCount,
                valueDescriptor
              ...
              while (copyValue < valueArgumentCount) limit INTERPRETER_LOCAL_WIDTH {
                long valueCallerArgument = localIndex(depth - 1, valueArgumentBase + copyValue);
                set(locals, localIndex(depth, copyValue), locals[valueCallerArgument]);
                if (
                  transferredType(readUnsigned(artifact, valueTypes + copyValue * 4, 4))
                ) {
                  set(locals, valueCallerArgument, 0);
                }
          ...
          if (opcode == OPCODE_CALL_VOID) {
            ...
            if (voidTarget < functionCount) {
              ...
              while (copyVoid < voidArgumentCount) limit INTERPRETER_LOCAL_WIDTH {
                long voidCallerArgument = localIndex(depth - 1, voidArgumentBase + copyVoid);
                set(locals, localIndex(depth, copyVoid), locals[voidCallerArgument]);
                if (
                  transferredType(readUnsigned(artifact, voidTypes + copyVoid * 4, 4))
                ) {
                  set(locals, voidCallerArgument, 0);
                }
          ...
          if (opcode < OPCODE_OWNED_MOVE) {} else {
            if (OPCODE_REGION_BORROW < opcode) {} else {
              ...
              match (storageStep) {
                ...
                case StorageStep.Value(long nextStorageCount, long nextStorageDataCursor) {
          ...
          if (opcode == OPCODE_SLICE_NEW) {
            ...
            if (
              aggregateWindowValid(aggregateCounts, sliceArrayHandle, sliceStart, sliceLength)
            ) {} else {
              return new ExecutionResult.Error(cursor);
            }
          ...
          if (opcode == OPCODE_VARIANT_NEW) {
            ...
            set(locals, localIndex(depth, variantDestination), variantAllocation.handle);
          }
          ...
          if (opcode == OPCODE_LOCAL_EQ) {
            ...
            if (
              locals[localIndex(depth, equalityLeft)] == locals[localIndex(depth, equalityRight)]
            ) {
              equalityValue = 1;
            }
          ...
          if (opcode == OPCODE_LOCAL_LT) {
            ...
            long lessValue = 0;
            if (
              locals[localIndex(depth, lessLeft)] < locals[localIndex(depth, lessRight)]
            ) {
              lessValue = 1;
            }
