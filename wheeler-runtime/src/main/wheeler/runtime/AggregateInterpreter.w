...
classical class AggregateInterpreter {
  ...
  public AggregateAllocation internAggregate(
    ...
    while (aggregate < aggregateCount) limit INTERPRETER_AGGREGATE_COUNT {
      ...
      while (field < fieldCount) limit INTERPRETER_LOCAL_WIDTH {
        if (
          aggregateFields[aggregateStarts[aggregate] + field] == locals[localBase + fieldBase
            + field]
        ) {} else {
          equal = false;
        }
  ...
  public long aggregateTag(borrow mut words aggregateTags, long handle) {
    if (handle < 1) {
      return -1;
    }
  ...
  public boolean aggregateFieldValid(borrow mut words aggregateCounts, long handle, long field) {
    if (handle < 1) {
      return false;
    }
