...
module wheeler.runtime.testing.test_artifact_pass_report;
...
classical class TestArtifactPassReport {
  private const long IDENTITY_BYTES = 32;
  private const long MAX_METADATA_BYTES = 255;
  private const long REPORT_FRAME_BYTES = 1190;
  private const long STAGING_BYTES = 101000;
  ...
  private long writeField(borrow byteview value, borrow mut bytes frame, long cursor) {
    ...
    assert(length < MAX_METADATA_BYTES + 1);
  ...
  /// Executes and reduces one passing classical artifact into a semantic report identity.
  public long derivePassingArtifactReportIdentity(
    ...
    region staging = new region(/* bytes= */ STAGING_BYTES, /* allocations= */ 10);
    bytes trace = allocateBytes(staging, MAX_INTERPRETED_STEPS * 2);
