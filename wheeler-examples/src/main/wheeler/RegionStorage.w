...
classical class RegionStorage {
  ...
  long writeWord(borrow mut words data, long index, long value) {
    set(data, index, value);
    return data[index];
  }

  long writeByte(borrow mut bytes data, long index, long value) {
    setByte(data, index, value);
    return data[index];
  }

  long readByte(borrow bytes data, long index) {
    return data[index];
  }

  long countScalars(borrow mut bytes data) {
