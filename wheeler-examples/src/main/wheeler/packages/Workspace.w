...
classical class Workspace {
    ...
    private boolean memberValid(
        ...
        if (keywordAt(
                source, starts, lengths, recordStart, 3217197722)) {
    ...
    private boolean nestedPath(
        ...
        if (parentLength < childLength) {
            ...
            while (offset < parentLength) limit 256 {
                if (utf8Scalar(source, starts[parent] + 1 + offset)
                        == utf8Scalar(source, starts[child] + 1 + offset)) {
                    offset += 1;
                } else {
                    return false;
                }
            ...
            return utf8Scalar(
                source, starts[child] + 1 + parentLength) == 47;
        }
        return false;
    }
    ...
    public WorkspaceResult parseWorkspace(
        ...
        boolean validName = validWorkspaceName(
            source, starts[1] + 1, lengths[1] - 2);
        boolean validProfile = validWorkspaceName(
            source, starts[3] + 1, lengths[3] - 2);
        if (keywordAt(
                source, starts, lengths, 0, 104652281998485)) {
                            ...
                            if (semicolonAt(source, kinds, starts, 4)) {
                                if (memberValid(
                                    ...
                                    if (memberValid(
                                            ...
                                            lengths,
                                            10)) {
                                        long memberOrder = compareTokenText(
                                            source,
                                            starts,
                                            lengths,
                                            6,
                                            11);
                                        boolean samePath = sameTokenText(
                                            source,
                                            starts,
                                            lengths,
                                            8,
                                            13);
                                        boolean firstContainsSecond = nestedPath(
                                            source, starts, lengths, 8, 13);
                                        boolean secondContainsFirst = nestedPath(
                                            source, starts, lengths, 13, 8);
                                        if (memberOrder < 0) {
                                            if (samePath) {
