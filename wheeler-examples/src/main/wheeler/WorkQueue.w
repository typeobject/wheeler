...
classical class WorkQueue {
  ...
  void observeEmpty(borrow mut words values, QueueCursor cursor) {
  ...
  void observeFull(borrow mut words values, QueueCursor cursor) {
  ...
  QueueCursor requirePush(borrow mut words values, QueueCursor cursor, long value) {
  ...
  QueueCursor takeFirst(borrow mut words values, QueueCursor cursor) {
    Pop result = pop(values, cursor);
    match (result) {
      case Pop.Empty() {
        return cursor;
  ...
  QueueCursor takeSecond(borrow mut words values, QueueCursor cursor) {
    Pop result = pop(values, cursor);
    match (result) {
      case Pop.Empty() {
        return cursor;
