...
classical class PlanIdentity {
  ...
  private long writeLength(borrow mut bytes output, long cursor, long length) {
  ...
  private long writeDecimalField(long value, borrow mut bytes output, long cursor) {
  ...
  private long writeKindField(long kind, borrow mut bytes output, long cursor) {
    if (kind == 1) {
