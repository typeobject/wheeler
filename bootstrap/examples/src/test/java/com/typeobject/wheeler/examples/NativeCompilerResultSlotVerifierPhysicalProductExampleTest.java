...
/** Native evidence for exact root byte-projection products. */
final class NativeCompilerResultSlotVerifierPhysicalProductExampleTest {
  @Tag("closure-evidence")
  @Test
  void compilesRootByteProjectionsByteForByte() throws Exception {
    var module = NativeCompilerArchiveClosureProgram.PHYSICAL_RESULT_SLOT_VERIFIER_MODULE;
    ...
    try {
      ...
      var frames = machine.snapshot().selectedFrames().stream()
          .map(frame -> {
            ...
            if (function.name().endsWith("module_symbols::indexCountedModuleSymbols")) {
              first = Math.min(403, frame.localCount());
              end = Math.min(440, frame.localCount());
            }
