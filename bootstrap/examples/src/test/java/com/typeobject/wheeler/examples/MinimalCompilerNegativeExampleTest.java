...
class MinimalCompilerNegativeExampleTest {
  ...
  void rejectsInvalidHelperResultsWithoutPublishingOutput() throws Exception {
    ...
    VirtualMachine reversibleLocalRightResult = new VirtualMachine(
        writerProgram,
        ("classical class ReversibleLocalRightResult { "
                + "rev long combine(long left, long right) { return left + right; } "
                + "entry void main() { } }")
            .getBytes(StandardCharsets.UTF_8),
        512);
    ...
    VirtualMachine threeParameterReversibleResult = new VirtualMachine(
        ...
        ("classical class ThreeParameterReversibleResult { "
                + "rev long answer(long left, long middle, long right) { return -1; } "
                + "entry void main() { long value = answer(1, 2, 3); } }")
            .getBytes(StandardCharsets.UTF_8),
        1024);
    assertTrapWithoutOutput(threeParameterReversibleResult, 1024);
  }
  ...
  void rejectsInvalidLocalStateUpdatesWithoutPublishingOutput() throws Exception {
    ...
    VirtualMachine unknownSignedLocalTarget = new VirtualMachine(
        writerProgram,
        ("classical class UnknownSignedLocalTarget { entry void main() { missing += 1; } }")
            .getBytes(StandardCharsets.UTF_8),
        512);
    assertTrapWithoutOutput(unknownSignedLocalTarget, 512);

    VirtualMachine booleanUpdateTarget = new VirtualMachine(
        writerProgram,
        ("classical class BooleanUpdateTarget { entry void main() { "
                + "boolean value = true; value += 1; } }")
            .getBytes(StandardCharsets.UTF_8),
        512);
    assertTrapWithoutOutput(booleanUpdateTarget, 512);

    VirtualMachine booleanLocalUpdateOperand = new VirtualMachine(
        writerProgram,
        ("classical class BooleanLocalUpdateOperand { entry void main() { "
                + "long value = 1; boolean delta = true; value += delta; } }")
            .getBytes(StandardCharsets.UTF_8),
        512);
