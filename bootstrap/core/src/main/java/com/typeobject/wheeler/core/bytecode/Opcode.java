...
public enum Opcode {
  ...
  RESULT_FILL_BINARY_SOURCES(
      OpcodeIds.RESULT_FILL_BINARY_SOURCES,
      InstructionForm.RESULT_BINARY_SOURCES,
      Reversibility.INTRINSIC),
  RETURN_RESULT_SLOT(
  ...
  public Opcode inverse() {
    return switch (this) {
      ...
      case RESULT_FILL_BINARY_SOURCES -> RESULT_FILL_BINARY_SOURCES;
  ...
  public boolean supportsGeneratedInverse() {
    return switch (this) {
      case ADD_CONST, SUB_CONST, XOR_CONST, SWAP, NOP, EXPECT_EQ, EXPECT_TRUE,
          ...
          RESULT_FILL_BINARY_SOURCES -> true;
      default -> false;
    };
  }
}
