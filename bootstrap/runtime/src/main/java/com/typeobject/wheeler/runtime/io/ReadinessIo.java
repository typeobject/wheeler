...
/** Bounded readiness queues that execute only requests with an asserted level signal. */
public final class ReadinessIo {
  private final int queueCount;
  private final int queueDepth;
  private long nextScopeId = 1;

  /** Creates fixed readiness queue topology. */
  public ReadinessIo(int queueCount, int queueDepth) {
  ...
  /** Opens one scope whose operation ceiling fits the readiness queues. */
  public synchronized IoScope scope(IoLimits limits) {
    ...
    if (nextScopeId == Long.MAX_VALUE) {
      throw new IllegalStateException("readiness scope identity exhausted");
    }
    return new IoScope(
        nextScopeId++,
        IoScope.Mode.READINESS,
        "bounded-readiness-io-1",
        limits,
        null,
        queueCount,
        queueDepth);
