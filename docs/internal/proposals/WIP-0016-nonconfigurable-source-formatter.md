...
- [x] The accepted syntax-owned table covers parenthesized parameter, argument, and record groups, bounded-loop conditions, array initializers, binary expressions, and indivisible tokens under deep indentation. Generated rename, insertion, removal, depth, and fit variants prove token preservation, smallest-group change, stable commas and extents, and idempotence. The formatter treats inequality as an operator predecessor, so a following signed literal remains tight. Tree-sitter models minus as a unary operator and accepts the same spaced token stream. The complete maintained Wheeler corpus remains canonical under the same table.
...
- [x] LF, one final newline, two-space indentation, tight unary operators, and fixed code layout derive only from formatter constants. Differential LF, CRLF, and CR fixtures produce identical canonical bytes. Inequality followed by a negative literal remains `!= -1`.
