...
- [x] The accepted syntax-owned table covers parenthesized parameter, argument, and record groups, bounded-loop conditions, array initializers, binary expressions, and indivisible tokens under deep indentation. Generated rename, insertion, removal, depth, and fit variants prove token preservation, smallest-group change, stable commas and extents, and idempotence. The complete maintained Wheeler corpus remains canonical under the same table.
