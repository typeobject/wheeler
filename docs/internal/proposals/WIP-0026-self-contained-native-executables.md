...
- [x] WIPs 0378 through 0390 and WIPs 0395 through 0407 lower one closed bounded scalar WBC profile, including up to twenty-three signed-, Boolean-, or void-result helpers with bounded forward and inverse recursion through depth sixty-four, sixteen exact scalar arguments per call, 256 locals and 512 instructions per ordinary function, checked loops through 4,096 iterations, status reads, helper-owned status publication, checked global updates, global equality expectations, swaps, logged replacement, forward checkpoint and commit markers, rotates, assertions, void helper calls, 4,096-byte constant output, 4,096-byte input-dependent native I/O, borrowed byte helpers, whole-execution fuel, strict UTF-8 input, and 32 shared scalar globals, to x86-64 Linux AOT and observe its computed source-declared process status through a complete ELF launch. General AOT equivalence remains.
