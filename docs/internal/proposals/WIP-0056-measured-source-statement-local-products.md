...
- [x] `SourceCallProducts.w` atomically binds each copied call token to the narrowest containing statement for its callable owner. Detached and equal-width ambiguous statement ranges leave caller rows untouched. `LoopCallProducts.w` consumes those statement identities when it merges exact physical widths.
- [x] `SourceOwnershipProducts.w` validates each effect against its statement's exact physical width and maps destination offsets, source value products, and instruction offsets through planned coordinates. Malformed effects leave ownership rows untouched. WIP-0057 owns decoded-evidence and archive joins.
