...
- [x] The 376-module measured symbol fixture publishes the complete declaration set with peak active source count one.
...
- [x] Private callable products stay local. Direct edges count only public dependency callables in header rank. WIP-0191 further excludes public callables whose complete signature is unavailable to the consuming source-product profile. Nominal peers remain available to aggregate compilation without poisoning primitive target views.
- [x] The physical compiler closure publishes 1,335 callable signatures with peak active source count one.
