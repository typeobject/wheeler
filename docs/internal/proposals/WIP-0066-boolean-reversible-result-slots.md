...
1. [ ] Specify the admitted `rev boolean` source forms and diagnostics.
2. [ ] Lower one preserved Boolean source into an explicit result-slot relation.
3. [ ] Publish Boolean presence and payload local-type rows.
4. [ ] Extend native function and instruction verification for Boolean payload slots.
5. [ ] Match stage 0 byte for byte through proof and artifact publication.
6. [ ] Execute forward and inverse after clearing history.
7. [ ] Reject signed sources, arithmetic relations, missing payloads, and mixed call results atomically.
8. [ ] Add the profile to the public language and bytecode references after implementation.
