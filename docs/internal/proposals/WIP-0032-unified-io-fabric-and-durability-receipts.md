...
- [x] WIP-0129 runs native one-sided writes through the portable request lifecycle. Exact target generation, target range, held source range, source-content identity, provider progress, and backend evidence gate private-constructor completion authority. Revocation races return uncertainty. Peer and persistence rows remain open.
...
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  ...
  - [ ] Add a native RNIC provider with registration, generation, range, rights, revocation, disconnect, acknowledgement, peer-application, and remote-persistence behavior matching the accepted rows.
    ...
    - [ ] Add one-sided reads, atomics, native cancellation, acknowledgement, peer-application, and remote-persistence evidence.
