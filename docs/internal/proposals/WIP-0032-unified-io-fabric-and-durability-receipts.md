...
- [x] WIP-0131 runs aligned native 64-bit compare-and-swap through `IoRequest`. Read-write rights, exact operands, observed value, provider progress, and backend evidence gate completion. Exchange success derives from the observed value. Native cancellation, peer, and persistence rows remain open.
...
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  ...
  - [ ] Add a native RNIC provider with registration, generation, range, rights, revocation, disconnect, acknowledgement, peer-application, and remote-persistence behavior matching the accepted rows.
    ...
    - [ ] Add native cancellation, acknowledgement, peer-application, and remote-persistence evidence.
