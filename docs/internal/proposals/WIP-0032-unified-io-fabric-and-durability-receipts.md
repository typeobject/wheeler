...
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  ...
  - [ ] Add native completion-port, aligned direct-I/O, storage-tier, and RNIC providers. Each must preserve the existing portable terminal rows and capability bounds.
