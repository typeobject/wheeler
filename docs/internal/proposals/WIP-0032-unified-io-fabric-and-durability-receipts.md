...
- [x] `CompletionIo` passes the common lifecycle through bounded one-lane and many-lane completion queues. Submission runs no provider code, queued cancellation releases its lane before effect, graph dependencies gate lane admission, and canonical reduction matches the deterministic backend. The stage-0 adapter allocates no worker, stack, task, or timer per queued operation. Native readiness and kernel completion adapters remain.
...
- [x] The declared host floor opens 256 simultaneous loopback connections with one service worker and runs 64 positional requests across four physical file queues through the common lifecycle. The semantic registry separately retains 100,000 dormant authorities without per-connection execution resources. This proves the checked floor on CI hosts, not a universal connection, IOPS, latency, protocol, or durability claim.
...
- [x] WIP-0128 adds bounded native RNIC registration authority. It binds held owner ranges, closed remote rights, monotonic generations, native addresses, and local and remote keys. Revocation and disconnect erase authority before backend teardown and release every owner after failure.
- [x] WIP-0129 runs native one-sided writes through the portable request lifecycle. Exact target generation, target range, held source range, source-content identity, provider progress, and backend evidence gate private-constructor completion authority. Revocation races return uncertainty.
- [x] WIP-0130 runs native one-sided reads through the same lifecycle. It holds the exact destination, validates generation and range twice around backend work, and hashes received bytes before owner return.
- [x] WIP-0131 runs aligned native 64-bit compare-and-swap through `IoRequest`. Read-write rights, exact operands, observed value, provider progress, and backend evidence gate completion. Exchange success derives from the observed value.
- [x] WIP-0132 assigns monotonic operation identities to native RNIC reads, writes, and atomics. Started cancellation names one backend operation. Queued cancellation invokes no backend hook and returns captured owners.
- [x] WIP-0133 orders native peer acknowledgement, peer application acceptance, and profile-bound remote persistence above one exact write completion. Every stage validates generation, predecessor, progress, and backend evidence twice around provider work. No stage promotes to `DurabilityReceipt`.
- [x] `QuantumIo` runs one target submission, wait, cancellation hook, typed result, terminal completion, and reap through `IoRequest` and `IoScope`. Construction allocates no target job, result identity is revalidated, and started cancellation stays acknowledged-partial or uncertain. One cancellation attempt owns the provider verdict. Result classification waits for that attempt to return, so observing the target's cancellation effect before its acknowledgement cannot publish false uncertainty. Hybrid acknowledged-job recovery still uses the interim `QuantumJob` adapter.
...
- [x] `wheeler.runtime.io.portable` owns nominal `Request`, `Scope`, `Operation`, closed `Completion`, bounded `CompletionQueue`, `EffectBoundary`, admission, completion publication, exact reaping, FIFO push/pop, and live-effect lowering. The executable fixture drives these values through the sole Wheeler lifecycle tables, rejects any inverse interpretation of external completion, closes only after reap, and rewinds the complete source machine. Java classes remain host adapters to these rows rather than the language spelling authority.
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  - [x] `NativePositionalFile` opens one no-follow bounded file capability with fixed rights, prepares pure positional reads and complete writes through ordinary `IoRequest`, holds buffers until terminal release, blocks close across unreaped resources, reports exact progress without a cursor, and survives close plus fresh read-only reopen. `openDirect` requires the host JDK `DIRECT` option, one power-of-two alignment, aligned file, owner-offset, and length coordinates, and native direct buffers cut with `alignedSlice` at that exact address boundary. It has no fallback path. Its private completion authority may issue only write-completed, data-stable, and file-stable receipts under an explicit one-replica process-crash profile containing the FileChannel force assumption. A power-loss profile rejects before receipt issuance.
  - [x] `NativeReadinessSocket` establishes one explicit connected capability, switches it to nonblocking mode, drives level readiness through `Selector.selectNow`, and returns read and write owners through ordinary readiness-gated requests. The same capability prepares caller-polled requests that perform no channel operation before `PollingIo.pollOne`. It allocates no per-operation worker, reports exact nonblocking progress, rejects close across unreaped resources, and releases queued cancellation without touching the socket.
  - [x] `NativeCompletionFile` binds positional host-file work to `AsynchronousFileChannel` over a fixed worker count and bounded executor queue. Pure requests submit no asynchronous operation before completion-queue progress, hold exact owners through terminal release, expose known progress and uncertainty through existing rows, reject close across unreaped resources, and let queued cancellation run no file operation.
  - [x] `NativeTieredStorage` places exact caller bytes through the asynchronous native file provider, reads one source placement under a captured transfer owner, verifies its content identity before target work, writes another named native tier, retains source evidence, and emits complete or exact-prefix ancestry through the ordinary completion row. Placement remains distinct from durability.
  - [ ] Qualify a concrete native RNIC backend against the accepted registration, transfer, cancellation, peer, and persistence rows.
    - [x] WIP-0128 implements bounded registration, generation, range, rights, revocation, and disconnect authority.
    - [x] WIP-0129 implements one-sided write transfer and exact native completion through `IoRequest`.
    - [x] WIP-0130 implements one-sided read transfer and exact received-content completion through `IoRequest`.
    - [x] WIP-0131 implements aligned 64-bit compare-and-swap and exact observed-value completion through `IoRequest`.
    - [x] WIP-0132 implements exact native operation identity and started-work cancellation through `IoRequest`.
    - [x] WIP-0133 implements peer acknowledgement, peer application acceptance, and profile-bound remote-persistence evidence.
    - [ ] Qualify named hardware, firmware, driver, cache, power, and replication profiles through the adapter.
- [ ] Crash and power-cut conformance qualifies file, namespace, replication, and remote-persistence evidence before a release-grade receipt issuer is enabled.
  ...
  - [x] `NativeNamespacePublication` consumes one file-stable named subject, performs one no-replacement `ATOMIC_MOVE` between physical siblings, issues namespace-visible evidence only after rename, opens and forces the containing directory, and then issues namespace-stable evidence. The exact visible receipt and physical target gate directory-force promotion. A child process forces file data, metadata, rename, and directory state, then terminates immediately through `Runtime.halt`. The parent observes only the exact target bytes through fresh process state. No file or namespace stage aliases another.
...
- [x] One-lane and many-lane completion, readiness, and explicit polling profiles pass the common stage-0 lifecycle. `InterruptIo` uses the fixed bounded dispatcher and terminal notification path. Queue admission precedes request consumption, cancellation needs no data-plane credit, readiness runs no provider before its level signal, polling follows canonical operation identity, and portable terminal fields agree.
...
- [x] The accepted source profile exposes physical host input and output authority only on `entry`. `setOutputLength` requires that exact entry output owner. `entry` is mutually exclusive with `rev`, `coherent rev`, `unitary`, and `test`, while theorem declarations have no executable body. Negative compiler evidence rejects ordinary helper, reversible, coherent, and unitary attempts before bytecode publication. Stage-0 `IoRequest` remains a host scaffold and is not an ambient source API.
