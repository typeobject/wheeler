...
- [x] WIP-0130 runs native one-sided reads through the same lifecycle. It holds the exact destination, validates generation and range twice around backend work, and hashes received bytes before owner return. Atomics, peer, and persistence rows remain open.
...
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  ...
  - [ ] Add a native RNIC provider with registration, generation, range, rights, revocation, disconnect, acknowledgement, peer-application, and remote-persistence behavior matching the accepted rows.
    ...
    - [ ] Add atomics, native cancellation, acknowledgement, peer-application, and remote-persistence evidence.
