...
- [ ] Native host positional, readiness, completion, polling, direct-I/O, network, storage, and RNIC adapters satisfy the accepted rows without widening authority.
  ...
  - [x] `NativeReadinessSocket` establishes one explicit connected capability, switches it to nonblocking mode, drives level readiness through `Selector.selectNow`, and returns read and write owners through ordinary readiness-gated requests. It allocates no per-operation worker, runs no provider action before readiness, reports exact nonblocking progress, rejects close across unreaped resources, and releases queued cancellation without touching the socket.
  - [ ] Add native completion-port, polling-driver, aligned direct-I/O, storage-tier, and RNIC providers. Each must preserve the existing portable terminal rows and capability bounds.
