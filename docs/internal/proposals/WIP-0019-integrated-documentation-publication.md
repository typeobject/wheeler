...
Wheeler builds one documentation graph from two named source types: authored Markdown manuals and attached Wheeler `//!` and `///` docs. A deterministic generator checks ownership, links, examples, proof references, package identities, and navigation. It then emits a renderer-neutral bundle.
...
Each source type keeps its authority. Markdown owns narrative manuals. Wheeler declarations and verified IR descriptors own Wheeler API reference. Java source and generated Javadoc are implementation material, not documentation graph inputs. Generated pages may link manuals and Wheeler declarations, but prose cannot turn a forward call into `rev`, a provider circuit into unitary evidence, or an absent method into a real declaration. Generated reference pages also cannot define a second meaning for `CALL_VALUE`.

Wheeler owns graph construction, safe Markdown rendering, navigation, output identities, and publication rules. Java remains a replaceable bootstrap seed until the generator is written in Wheeler. It does not receive a parallel documentation namespace.
...
- implementation comments will acquire public routes and shadow Wheeler API anchors.
...
2. A stage-0 Java class implements the canonical bytecode reader. It remains visible in repository source and maintainer evidence, but cannot define a public documentation node. Deleting Java at native cutover therefore changes no bundle identity or Wheeler symbol URL.
...
5. Rendering fails after bundle generation but before publication. The immutable documentation bundle remains complete and content-addressed. Retrying rendering consumes the same verified bundle. It does not regenerate examples, proof checks, or package resolution.
...
- Combine authored Markdown and Wheeler declaration docs without duplicate semantic authorities.
...
- Exclude Java implementation APIs from public documentation graph inputs.
- Permit renderer retry without rerunning semantic generation.
- Port the generator to Wheeler and delete its Java implementation at native cutover.

## Non-goals

- Make prose, implementation comments, or a rendered page authoritative for language semantics, bytecode, proofs, or packages.
- Parse arbitrary Markdown, arbitrary HTML, Java source, generated Javadoc, themes, or plugins.
...
A **documentation source** is one bounded, strict-UTF-8 input with an exact content identity and source kind: `manual`, `wheeler-api`, `generated-reference`, or `asset`.

A **documentation node** is a canonical typed record representing a manual page, section, Wheeler declaration, theorem reference, executable example, package, version, or asset.

A **documentation edge** is a typed relation such as `contains`, `documents`, `links-to`, `proved-by`, `example-of`, `supersedes`, or `version-of`.
...
### Stage-0 Java exclusion

Java source, Java comments, standard-doclet output, and generated Javadoc HTML are not documentation graph inputs. They may appear in internal maintainer evidence or ordinary source links. They cannot create routes, graph nodes, navigation entries, search rows, sitemap URLs, package exports, or Wheeler declaration aliases.

The bundle collector accepts only explicit Markdown/MDX roots and `.w` API roots. Adding or deleting a Java subsystem therefore changes no public graph identity unless a maintained manual or Wheeler declaration changes with it.
...
Exact package releases and lock identities version package documentation. The site may expose aliases such as `latest`, but aliases resolve in a separately signed publication manifest and never enter stored links or proof claims.
...
## Java and Javadoc boundary

Java source and Javadoc remain outside canonical collection. The generator does not invoke a doclet, scrape HTML, normalize Java signatures, or reserve Java routes. This is a source-selection rule, not a filter applied after graph construction.

Maintainers may run ordinary Javadoc for local implementation work. Such output is unpublished, unversioned by the documentation manifest, and incapable of satisfying Wheeler API coverage.
...
Complete source, tool, policy, and example identities key each cache entry. Cache corruption fails digest verification and triggers recomputation. Stale data is rejected.
...
The semantic build bounds source files, bytes, nodes, edges, links, nesting, includes, generated pages, examples, diagnostics, assets, search terms, output bytes, and total work.
...
The compiler and WIP-0016 own Wheeler declaration/document attachment. The Markdown parser owns the accepted manual syntax. The documentation generator owns graph validation, identities, links, navigation, examples, and bundle publication. Its current Java implementation is quarantined under `bootstrap/`. The canonical destination is a Wheeler tool package that reproduces the bundle byte-for-byte before the Java generator is deleted.
...
4. Prove that Java source and generated Javadoc cannot enter or shadow the canonical graph.
...
10. Delete the Java generator after Wheeler parity. Keep no copied API Markdown, Java extraction path, duplicate navigation, renderer package lock, or obsolete build script.
...
- [x] Wheeler API nodes consume WIP-0016's parser-owned module, declaration, modifier, summary, facet, and source-position records through the shared byte-oriented tooling boundary. Validation and extraction use one lossless scan. The bundle generator does not rescan or normalize `//!` and `///` payloads.
