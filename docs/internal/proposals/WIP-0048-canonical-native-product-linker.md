...
The focused fixtures establish linker behavior. Complete physical compiler emission and stage equality close the self-hosting integration gate. Recovery promotion still requires the independent derivation described below.
...
- [ ] WIP-0049 publishes one source-local artifact for every physical compiler module.
  The retained subset already passes source-local comparison, counted framing,
  identity-based relocation, string/type/function emission, independent verification,
  and execution of its synthetic library entry. See the
