...
- [x] `CountedAggregateLayouts.w` appends one validated artifact or descriptor-compatible source product at a time into 4,096 aggregate, 8,192 case, and 16,384 member rows. It rejects duplicate module owners and rebases case and member windows without dependency source. WIP-0173 publishes measured source and projected rows. WIP-0174 copies only those rows into counted local staging.
