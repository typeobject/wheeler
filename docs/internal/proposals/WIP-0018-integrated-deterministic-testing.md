...
- [ ] An end-to-end package suite compiles and tests the complete self-hosted compiler using no Java discovery. WIP-0279 proves the package command and native case-row path over a seven-module physical compiler spine. WIP-0292 adds a second seven-module physical compiler partition. WIP-0293 adds three call-syntax and loop-layout modules, raising the checked-in suite to seventeen native cases. WIP-0294 binds bounded dependency archives and their manifests to exact lock rows before external test sources are admitted. The complete source graph and external source projection remain.
