...
- [x] Terminal 255-case discovery, parameter-row admission, execution, and case-256 rejection have separate evidence methods. The complete execution attempt runs once under an explicit three-minute host cancellation guard while Wheeler's semantic step limits remain authoritative.
- [x] The 228-case physical compiler package suite runs through the dedicated `:tools:nativeCompilerPackageTest` integration task. Ordinary tools checks exclude that one long host launcher. CI waits for both ordinary build matrices, then executes the suite once on Temurin with a forty-seven-minute method bound inside the fifty-minute job. Duplicate runs cannot starve bounded example shards.
