...
- [x] The 243-case physical compiler package suite runs through the dedicated `:tools:nativeCompilerPackageTest` integration task. Ordinary tools checks exclude that launcher. WIP-0412 assigns complete case identities to sixteen disjoint Temurin jobs after both ordinary build matrices pass. Each shard selects before compilation, executes once with fresh storage, and has a twelve-minute method bound inside an eighteen-minute job. The Java publisher no longer repeats Wheeler's case inventory.
