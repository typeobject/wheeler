...
- [ ] Native no-Java recovery uses only committed manifests, locks, and workspace sources or an exported verified vendor closure. WIP-0269 validates complete schema-3 lock structure and ordering in the native test path. WIP-0270 binds direct manifest dependency names to that package set. WIP-0271 checks exact, caret, and tilde stable constraints against locked versions. WIP-0272 adds complete prerelease grammar, precedence, and stable-range exclusion. WIP-0273 requires every locked dependency edge to name a package entry. WIP-0274 rejects cycles and packages unreachable from direct manifest dependencies. WIP-0275 invokes the native package gate with physical nonempty locks when selected test imports remain package-local. WIP-0276 publishes complete native case rows and removes Java discovery and execution from eligible package commands. External import provenance remains.
...
- [x] Catalog or manifest insertion order does not change resolution, and explicit limits bound backtracking. Configured repository order is exact. The first authoritative repository with an admissible release wins, without cross-repository version mixing. Filesystem entries are sorted before they reach the solver.
...
- [x] Build-plan codecs canonicalize graph order and reject corruption or forged node identities. Direct and sealed clean builds produce byte-identical `.wbc`, and a forged active-compiler identity is rejected. Full native reproduction of locks, archives, and provenance remains.
- [x] Vendored input relocation preserves exact locked bytes and rejects extra, linked, missing, mismatched, or poisoned files. XDG repository paths are identity-neutral. Package and build-output cache poisoning, deletion, PREV divergence, immutable object reuse, quarantine, and garbage collection are tested. Identity-preserving network mirrors remain.
