...
Every source-copy and declaration-blanking loop uses the complete physical bound.
[WIP-0497](WIP-0497-exact-source-word-admission.md) replaces the token-hash wrapper
with exact source-word classification. Words longer than the thirteen-byte fixed
vocabulary return zero before traversal. That bound limits neither identifiers
nor comments, and long comments cannot exhaust a keyword loop.
