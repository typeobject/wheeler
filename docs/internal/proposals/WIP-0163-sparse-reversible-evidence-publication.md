...
`NativeCompilerPhysicalClosureExampleTest` now compares all 119 selected artifacts, retained prefixes, and relocations. WIP-0411 adds the direct assignment-syntax product and closes the wide-local dependency named by `CallForms.w`. WIP-0413 adds the guarded UTF-8 call owner without a broad token-module edge. WIP-0414 retains separate statement, decoder, signed, and nonsigned result owners. WIP-0415 retains the manifest assertion classifier. WIP-0416 retains the profile classifier after admitting Boolean source children. WIP-0420 retains the package-manifest token product, WIP-0421 retains the package-name validators, WIP-0422 retains the package-path validators, WIP-0424 retains semantic-version scalar and core validation, WIP-0425 retains prerelease validation with imported core calls, WIP-0426 retains semantic-version coordinates, WIP-0427 retains identifier precedence, WIP-0428 retains core precedence, WIP-0429 retains prerelease sequence order, WIP-0430 retains release precedence, and WIP-0431 closes the semantic-version facade. Each fresh test run links 387 functions and 14,082 instructions, retains 10,808 local types and 334,288 code bytes, and reproduces SHA-256 `9906585f7d46df682949ba4dc1add7a3bf85800caa33a488a4feb87d1271acc7`. Complete evidence remains bounded by the forty-five-minute method deadline.
