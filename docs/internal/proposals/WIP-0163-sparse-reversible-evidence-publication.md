...
`NativeCompilerPhysicalClosureExampleTest` covers retained prefixes, relocation,
and linked-container publication. Current closure evidence belongs in the
[self-hosting status map](self-hosting-status.md), not in this completed
publication record. The archive identity above records this WIP's implementation
milestone.
