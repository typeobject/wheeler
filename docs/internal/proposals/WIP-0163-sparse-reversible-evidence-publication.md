...
`NativeCompilerPhysicalClosureExampleTest` now compares all 112 selected artifacts, retained prefixes, and relocations. WIP-0411 adds the direct assignment-syntax product and closes the wide-local dependency named by `CallForms.w`. WIP-0413 adds the guarded UTF-8 call owner without a broad token-module edge. WIP-0414 retains separate statement, decoder, signed, and nonsigned result owners. WIP-0415 retains the manifest assertion classifier. WIP-0416 retains the profile classifier after admitting Boolean source children. WIP-0420 retains the package-manifest token product, WIP-0421 retains the package-name validators, WIP-0422 retains the package-path validators, and WIP-0424 retains semantic-version scalar and core validation. Each fresh test run links 333 functions and 12,567 instructions, retains 9,380 local types and 297,352 code bytes, and reproduces SHA-256 `267f7873478bfb267d17a9255df99bf745b12b91a8a57c79d3707d3272e6b930`. Complete evidence passes in about 19 minutes under the twenty-four-minute method deadline.
