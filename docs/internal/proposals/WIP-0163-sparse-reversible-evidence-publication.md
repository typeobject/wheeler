...
`NativeCompilerPhysicalClosureExampleTest` now compares all 113 selected artifacts, retained prefixes, and relocations. WIP-0411 adds the direct assignment-syntax product and closes the wide-local dependency named by `CallForms.w`. WIP-0413 adds the guarded UTF-8 call owner without a broad token-module edge. WIP-0414 retains separate statement, decoder, signed, and nonsigned result owners. WIP-0415 retains the manifest assertion classifier. WIP-0416 retains the profile classifier after admitting Boolean source children. WIP-0420 retains the package-manifest token product, WIP-0421 retains the package-name validators, WIP-0422 retains the package-path validators, WIP-0424 retains semantic-version scalar and core validation, and WIP-0425 retains prerelease validation with imported core calls. Each fresh test run links 351 functions and 13,089 instructions, retains 9,862 local types and 310,008 code bytes, and reproduces SHA-256 `9a91bc4b32452c0248627b6b3c7973390e3efdb2f527aa834b64e29aef854c95`. Complete evidence passes in about 20 minutes under the twenty-four-minute method deadline.
