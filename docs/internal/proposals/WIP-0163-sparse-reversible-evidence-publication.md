...
`NativeCompilerPhysicalClosureExampleTest` now compares all 117 selected artifacts, retained prefixes, and relocations. WIP-0411 adds the direct assignment-syntax product and closes the wide-local dependency named by `CallForms.w`. WIP-0413 adds the guarded UTF-8 call owner without a broad token-module edge. WIP-0414 retains separate statement, decoder, signed, and nonsigned result owners. WIP-0415 retains the manifest assertion classifier. WIP-0416 retains the profile classifier after admitting Boolean source children. WIP-0420 retains the package-manifest token product, WIP-0421 retains the package-name validators, WIP-0422 retains the package-path validators, WIP-0424 retains semantic-version scalar and core validation, WIP-0425 retains prerelease validation with imported core calls, WIP-0426 retains semantic-version coordinates, WIP-0427 retains identifier precedence, WIP-0428 retains core precedence, and WIP-0429 retains prerelease sequence order. Each fresh test run links 382 functions and 14,008 instructions, retains 10,722 local types and 332,472 code bytes, and reproduces SHA-256 `35c6345ec836a7b5c35ef63a67170218ba579a2d6d160a5c60cfa9dfc93b5695`. Complete evidence remains bounded by the forty-five-minute method deadline.
