...
`NativeCompilerPhysicalClosureExampleTest` now compares all 111 selected artifacts, retained prefixes, and relocations. WIP-0411 adds the direct assignment-syntax product and closes the wide-local dependency named by `CallForms.w`. WIP-0413 adds the guarded UTF-8 call owner without a broad token-module edge. WIP-0414 retains separate statement, decoder, signed, and nonsigned result owners. WIP-0415 retains the manifest assertion classifier. WIP-0416 retains the profile classifier after admitting Boolean source children. WIP-0420 retains the package-manifest token product, WIP-0421 retains the package-name validators, and WIP-0422 retains the package-path validators. Each fresh test run links 317 functions and 12,120 instructions, retains 8,993 local types and 286,600 code bytes, and reproduces SHA-256 `2cbec7e1bedc1c39753c557b4bf38fc367e1fb72c904aaf1c6759ebc70b880df`. Complete evidence passes in about 19 minutes under the twenty-four-minute method deadline.
