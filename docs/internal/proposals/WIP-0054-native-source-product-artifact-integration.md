...
- [x] The physical `CoreParsing.w` source publishes 25 statements, seven blocks, two loops, 15 owner-scoped values, 14 direct loop leaves, three nested guards, and both named 4,096-iteration limits. Structured value resolution accepts explicit row coordinates. Callable roots no longer depend on global block zero.
