...
- [x] The physical `CoreParsing.w` source publishes 25 statements, seven blocks, two loops, 15 owner-scoped values, 14 direct loop leaves, three nested guards, and both named 4,096-iteration limits. Its complete loop instruction windows and loop-local type suffixes match stage 0 byte for byte. Structured value resolution follows source ordinals, measures indexed reads, accepts explicit row coordinates, and treats each callable root independently.
