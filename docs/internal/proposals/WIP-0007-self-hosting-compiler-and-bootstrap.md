...
- [x] WIP-0378 adds the first verified native lowering leaf. One canonical three-instruction WBC writes a bounded status global, lowers to import-free x86-64 Linux text, and reaches that source-declared process status through a complete AOT ELF launch. It is a stage-0 backend slice, not the self-hosted compiler or general classical profile.
