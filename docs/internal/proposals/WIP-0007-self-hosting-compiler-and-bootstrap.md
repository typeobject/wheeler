...
- [x] WIPs 0378 through 0390 add the first verified native lowering leaf. One canonical bounded scalar WBC computes a bounded status global through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to seven prior helpers, six exact scalar arguments per call, checked loops through 4,096 iterations, status reads, rotates, assertions, void helper calls, 4,096-byte constant output, 4,096-byte input-dependent native I/O, borrowed byte helpers, whole-execution fuel, strict UTF-8 input, and 32 shared scalar globals, lowers to import-free x86-64 Linux text, and reaches that source-declared process status through a complete AOT ELF launch. It is a stage-0 backend slice, not the self-hosted compiler or general classical profile.
