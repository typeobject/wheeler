...
- [x] WIPs 0378 and 0379 add the first verified native lowering leaf. One canonical acyclic scalar WBC computes a bounded status global through moves, checked arithmetic, bitwise operations, comparisons, forward branches, and up to seven prior zero-argument helper calls, lowers to import-free x86-64 Linux text, and reaches that source-declared process status through a complete AOT ELF launch. It is a stage-0 backend slice, not the self-hosted compiler or general classical profile.
