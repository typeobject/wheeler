...
- [ ] Transactional persistent index.
  - [x] `TransactionalPersistentIndex.w` stages a copy-on-write root while the old root remains visible, writes root and sequence before the commit marker, records applied transaction identities in the locked deterministic map, rejects duplicate application through a tagged result, injects one marker-free torn record, and scans a bounded log to reopen the latest committed root.
  - [ ] Run the same ordering and recovery protocol over a native temporary-file capability, force the root and marker at distinct durability boundaries, terminate between them, and reopen through a fresh process.
...
- [ ] Amplitude estimation.
  ...
  - [ ] Add controlled coherent amplitude-operator calls and repeated powers to the source fixture, then compare the bounded estimator against the exact amplitude over more than one known distribution.
