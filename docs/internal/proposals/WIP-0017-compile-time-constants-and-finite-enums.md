...
- [x] `FiniteEnumPermutation` checks one complete semantic case-name map over the canonical payload-free variant authority. It rejects missing, unknown, duplicate, payload-bearing, and noncanonical case products, derives the exact inverse, and gives type and mapping separate content identities. The conformance corpus accepts all twenty-four four-case permutations. Typed `rev` enum source signatures remain with WIP-0031.
- [x] The same checked product admits coherent use only for an exact power-of-two cardinality. Canonical case-name order fixes basis position and qubit width. Forward amplitude permutation matches an independent oracle, and the generated adjoint restores exact real and imaginary amplitudes.
...
- [x] Power-of-two coherent enum permutations agree with an independent amplitude oracle, and generated adjoints restore every amplitude exactly.
