...
- [x] The Wheeler recovery compiler evaluates one contiguous block of up to 256
  scalar constants around optional signed state. Forward references, checked
  arithmetic, Boolean expressions, rotates, and typed scalar uses match stage 0.
  Evaluation allows at most 4,096 steps per lookup, sixty-four dependency levels,
  and thirty-two parenthesis levels. Split blocks, cycles, unknown names, type
  mismatches, arithmetic traps, and excess declarations publish nothing.
- [x] WIP-0043 replaces topology-specific execution with complete bounded graph
  plans. Public constants resolve through direct imports in canonical dependency
  order. Private declarations do not leak, and duplicate exports fail. The
  [compiler catalog](catalog/compiler.md) records graph and callable extensions.
  Full physical compiler integration remains under WIP-0007 and WIP-0044.
...
- [x] `FiniteEnumPermutation` checks one complete semantic case-name map over the canonical payload-free variant authority. It rejects missing, unknown, duplicate, payload-bearing, and noncanonical case products, derives the exact inverse, and gives type and mapping separate content identities. The conformance corpus accepts all twenty-four four-case permutations. `wheeler.core.finite_permutations` independently validates and applies counted permutation rows, publishes inverses atomically, and rejects malformed rows without changing output. Typed `rev` enum source signatures remain with WIP-0031.
- [x] The same checked product admits coherent use only for an exact power-of-two cardinality. Canonical case-name order fixes basis position and qubit width. Forward amplitude permutation matches an independent oracle, and the generated adjoint restores exact real and imaginary amplitudes. The Wheeler implementation reproduces forward, inverse, exact split-amplitude, adjoint, width, and basis-measurement products without Java state.
...
- [x] Exhaustive four-case evidence accepts every finite permutation and rejects duplicate, missing, unknown, payload-bearing, and noncanonical products before publishing an identity.
- [x] Power-of-two coherent enum permutations agree with an independent amplitude oracle, and generated adjoints restore every amplitude exactly. The Wheeler implementation differentially reproduces the complete forward, inverse, split-amplitude, adjoint, qubit-width, and exact basis-measurement products.
- [x] Non-power-of-two coherent use fails before output-state allocation or mutation.
- [x] Exact basis measurement returns a distinct nominal `MeasuredCase`. It is not a `CoherentState`, its constructor is private, and its API has no inverse or `uncall` operation.
