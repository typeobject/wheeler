...
- [x] The accepted target profile closes selected operation capabilities, circuit and register ownership, preparation basis width, inverse eligibility, register width, shot count, and parameter schema before submission. Missing dynamic capabilities, mismatched registers, oversized basis values, dynamic inverse requests, excessive qubits or shots, and mismatched symbolic bindings fail before a provider job identity is allocated. Batch submission validates every member's closed requirement set before submitting the first member.
