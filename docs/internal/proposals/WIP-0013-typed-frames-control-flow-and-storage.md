...
- [x] Bounded regions enforce byte and object ceilings, affine moves, leak-free exits, mutable signed-word and byte buffers, immutable validated UTF-8 owners with read-only parameter borrows, exclusive region scratch-allocation borrows, and word, byte, and map mutable borrows. Primitive owners transfer into callees or return across frames. Owner relays, caller-region factories, explicit drop order, use-after-call rejection, snapshots, and rewind are differential-tested. Returned loans, split and join borrowing, typed collections, recoverable allocation, capabilities, and compiler-scale arenas remain separate extensions.
...
- [x] The current call profile covers exact signed and Boolean argument transfer, primitive owner parameters and results, recursive execution, 1,024-frame exhaustion, inverse scalar signatures, and rewind. Returned loans, aggregate move or loan calls, and nested trap propagation remain.
- [x] Branch tests cover both paths, join assignment, early typed return, bounded nested blocks, and source diagnostics. Unreachable-block diagnostics remain.
...
- [x] Region tests cover word, byte, and map allocation and mutation, byte ranges, strict UTF-8 boundaries, borrow kind and aliasing, scratch cleanup, capacity failure, drop order, moved values, leaks, ownership joins, canonical encoding, snapshots, and rewind. Dangling returned borrows, output-address independence, recoverable allocation, and region-scoped commit remain.
