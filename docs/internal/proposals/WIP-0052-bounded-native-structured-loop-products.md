...
- [x] `LoopBodyInstructionEncoding.w` owns direct-statement byte encoding and local widths for outer and nested loop products. Borrowed buffer reads, writes, and indexed copies emit owner temporaries. Owned buffers retain the shorter form. The old nested-block update opcode dialect is gone.
