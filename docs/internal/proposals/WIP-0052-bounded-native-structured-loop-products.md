...
- [x] `ResolvedLoopBodyProducts.w` publishes direct signed declarations, Boolean literal and signed-local equality declarations, type-checked signed and Boolean assignments, Boolean assertions, local-to-literal, literal-to-local, and local-to-local signed comparisons, checked local-update, borrowed-word read/write flow, mutable-byte read/write flow, and immutable byte-view reads with exact monotonic local bases. Named sources require one visible prior value. Unsupported, cross-type, malformed-index, wrong-owner, or ambiguous body rows publish nothing. The borrowed-word fixture presets the caller row and proves failed reads and writes leave the count and sentinel unchanged. A complete source-product artifact fixture covers byte-view reads, mutable-byte reads and writes, byte-buffer copies, byte-view-to-buffer copies, parameter loans, exact local coordinates and types, verification, and byte-for-byte stage-0 equality.
