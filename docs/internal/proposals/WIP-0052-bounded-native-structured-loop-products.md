...
- [x] The current nested-block profile joins one-arm signed local equality, less-than, and Boolean guards with measured direct child products. Boolean assignment and buffer-write children match stage 0. `LoopNestedBlockProducts.w` rejects detached or recursively nested children atomically. `LoopCallProducts.w` publishes source-independent call code, types, and relocations. `LoopBackEdgeProducts.w` validates ownership state and back edges. Deeper reversible block forms remain with WIP-0035.
