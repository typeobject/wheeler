...
- [x] The accepted source-package artifact path ignores checkout location, file timestamp, default locale, default timezone, and input-map order in one focused command test. The dual-builder CI produces and compares the complete canonical artifact set with one versus two Gradle workers, distinct JDK distributions, and distinct timezones. Canonical content identities therefore follow the bytes and closed plan rather than these ambient facts.
