...
- [x] WIP-0378 adds the first verified WBC-to-x86-64 lowering leaf. One closed straight-line scalar program computes a bounded source-declared status through moves, checked arithmetic, and bitwise operations, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
