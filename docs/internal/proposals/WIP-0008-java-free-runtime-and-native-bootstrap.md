...
- [x] WIPs 0378 through 0390 and WIPs 0395 through 0408 add the first verified WBC-to-x86-64 lowering leaf. One closed bounded scalar program computes a bounded source-declared status through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to twenty-three signed-, Boolean-, or void-result helpers with bounded forward and inverse recursion through depth sixty-four, sixteen exact scalar arguments per call, 256 locals and 512 instructions per ordinary function, checked loops through 4,096 iterations, status reads, helper-owned status publication, reversible signed and Boolean result slots, checked global updates, global equality expectations, swaps, logged replacement, forward checkpoint and commit markers, rotates, assertions, void helper calls, 4,096-byte constant output, 4,096-byte input-dependent native I/O, borrowed byte helpers, whole-execution fuel, strict UTF-8 input, and 32 shared scalar globals, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
