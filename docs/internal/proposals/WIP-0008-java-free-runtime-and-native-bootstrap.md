...
- [x] WIPs 0378 through 0383 add the first verified WBC-to-x86-64 lowering leaf. One closed bounded scalar program computes a bounded source-declared status through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to seven prior helpers, six exact scalar arguments per call, checked loops through 255 iterations, status reads, rotates, assertions, and void helper calls, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
