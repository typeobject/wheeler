...
- [x] WIP-0378 adds the first verified WBC-to-x86-64 lowering leaf. One closed three-instruction program writes a bounded source-declared status and returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
