...
- [x] WIPs 0378 through 0387 add the first verified WBC-to-x86-64 lowering leaf. One closed bounded scalar program computes a bounded source-declared status through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to seven prior helpers, six exact scalar arguments per call, checked loops through 255 iterations, status reads, rotates, assertions, void helper calls, 4,096-byte constant output, 4,096-byte input-dependent native I/O, borrowed byte helpers, and whole-execution fuel, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
