...
- [x] WIPs 0378 through 0381 add the first verified WBC-to-x86-64 lowering leaf. One closed bounded scalar program computes a bounded source-declared status through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to seven prior helpers, six exact scalar arguments per call, and checked loops through 255 iterations, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
