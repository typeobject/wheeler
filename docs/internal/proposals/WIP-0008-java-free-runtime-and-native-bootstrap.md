...
- [x] `NativeVm.w` and `runtime/Interpreter.w` execute the verified bounded compiler profile inside Wheeler. WIP-0286 moves canonical section, descriptor, frame, parameter, and instruction coordinates into the focused `InterpreterLayout.w` authority.
