...
- [x] WIP-0373 exposes that adapter through physical build and verify commands with strict metadata parsing, complete WBC verification, output self-verification, and atomic publication.
- [x] WIP-0374 adds canonical static arm64 Mach-O layout and verification. Separate R-X runtime and R-- capsule segments, exact entry state, fixed platform version, canonical rebuilding, and unsigned PREV are enforced. Wheeler runtime machine code, host shims, Apple signing, and native launch remain.
