...
- [x] WIPs 0378 through 0390, WIP-0395, and WIP-0396 add the first verified WBC-to-x86-64 lowering leaf. One closed bounded scalar program computes a bounded source-declared status through moves, checked arithmetic, bitwise operations, comparisons, forward branches, up to seven helpers in one acyclic call graph, seven exact scalar arguments per call, checked loops through 4,096 iterations, status reads, rotates, assertions, void helper calls, 4,096-byte constant output, 4,096-byte input-dependent native I/O, borrowed byte helpers, whole-execution fuel, strict UTF-8 input, and 32 shared scalar globals, then returns it through Linux process exit. Distinct WBC values produce distinct native observations without fallback.
