...
test void signed(long value) cases(-1, 0, 1) tags(unit, signed.edge)
    limits(steps = 1000, history = 1000) { ... }
entry void main() { ... }
```
...
The recovery compiler substitutes evaluated values into matching scalar locals, direct helper returns, scalar assignments, checked signed updates including generated reversible helper updates, one- or two-argument scalar helper calls, right operands of signed arithmetic and ordering expressions or signed and Boolean equality and inequality expressions, signed arithmetic returns, typed comparison returns over signed or Boolean operands, signed or Boolean equality assertions, signed ordering assertions, conditions and their state-update values, plus bounded loop conditions and limits and affine-region byte and allocation limits. One-arm equality and ordered literal guards may assign a Boolean literal to a prior Boolean local. Ordered guards accept either local-literal or literal-local spelling. Calls and mutations may mix constants and prior locals. Helper parameters and locals cannot reuse constant names. Constants create no runtime object or source-order artifact. Native token hashing masks each multiplication input to fifty-eight bits and accepts up to 256 scalars, which keeps checked arithmetic out of the ditch for longer identifiers. An entryless library with zero or one general helper receives the unqualified `$library` halt entry. A closed helper-table form accepts one through twenty-three public, private, or unqualified zero- through sixteen-parameter helpers and emits their complete canonical function table. One fixed-array helper uses that table without a dummy declaration. Zero is empty, one is not indecisive. Parameters may mix signed values with shared UTF-8 or byte-view loans and mutable byte, word, region, or signed-map loans. A signed or Boolean local may initialize from an exact zero- through two-argument same-module or direct imported call with matching primitive argument types. Each loan is reborrowed into the call frame. The native recovery subset admits one live `bytes` allocation through a `borrow mut region`, followed by explicit `drop` before return. It emits the canonical region and length moves, `BYTES_ALLOC`, owned move, type rows, and `BUFFER_DROP`. A wrong source type, leaked owner, foreign drop, or second drop fails before publication. An invalid entryless scalar library is final. The parser does not shop it around the entry-bearing helper profile until one forgets to object. The fixed-width core readers use the borrowed two-argument form without smuggling the byte view through a signed move. A signed local may also call an exact three- through seven-parameter helper with the same number of prior primitive locals. Every loan is reborrowed with its canonical opcode, and `CALL_VALUE` names the resolved function. Five through seven sources use two bounded packed operands rather than opcode arithmetic. Wrong primitive types, arity, result kind, a literal in a wide-local form, or an eighth argument publishes nothing. An existing signed local may also receive a same-module or direct imported signed-helper result over zero through seven exact prior primitive locals. The assignment evaluates each source once, reborrows loans by type, and moves the result into the existing target. Boolean targets, mismatched source order, literals, and eight arguments fail before publication. A bounded final call may forward one through seven matching signed values, fixed-array owners, or loans within the module or into a direct import. `ReturnCallCodegen.w` emits the whole family through one bounded path. Seven arguments use fifteen locals and sixteen instructions. An eighth fails before publication. A signed helper may return `bufferLength` directly or bind it to a signed local from UTF-8, byte-view, byte, or word loans. It may also bind or directly return `utf8Scalar` or `utf8Width` from a UTF-8 loan and signed index, bind one indexed byte or word from a byte-view, byte, or word loan, or return that element directly without inventing a local solely to appease the backend. Mutable byte and word loans admit `setByte` and `set`, while mutable signed-map loans admit `put`. Each uses signed key or index and value locals. Signed and Boolean helpers may bind or directly return `mapGet` and `mapHas` from a signed-map loan and signed key. The native helper profile also accepts signed fixed-array owners with one through sixty-four elements. It assigns up to sixteen distinct lengths canonical descriptors in first declaration order, moves those owners through matching same-module or direct imported calls, and lowers indexed reads to `ARRAY_GET`. Length seventeen gets a descriptor. Distinct type seventeen does not. Entryless void helpers accept zero through sixteen primitive parameters and either an empty body or those writes. Their type table has no imaginary result local. Void and scalar-result helpers may issue zero- through seven-argument calls to same-module or direct imported void helpers with exact primitive types and canonical reborrows. Four through seven sources use two bounded packed operands. Eight fail before publication. Other intrinsic loan reads and writes remain outside the native subset. The native compiler reproduces stage 0 for the checked-in `compiler/backend/calls/CallArguments.w`, `compiler/backend/EncodingWidths.w`, imported-constant `compiler/frontend/intrinsics/BorrowedIntrinsicShapes.w`, `compiler/ir/Opcodes.w`, `compiler/ir/ProofRules.w`, `compiler/ir/ResolvedStatements.w`, `compiler/ir/StatementKinds.w`, `compiler/ir/StorageOpcodes.w`, `compiler/ir/TypeCodes.w`, `compiler/ir/limits/CompilerProgramLimits.w`, imported-function `compiler/verification/ResultSlotVerifier.w`, imported-constant `compiler/resolution/returns/ReturnOpcodeKinds.w`, imported-constant `compiler/syntax/assignments/NamedLocalAssignmentKinds.w`, imported-constant `compiler/syntax/assignments/ResolvedLocalAssignments.w`, imported-constant `compiler/syntax/assertions/ResolvedBooleanLiteralAssertions.w`, imported-constant `compiler/syntax/assertions/ResolvedLessThanAssertions.w`, imported-constant `compiler/syntax/assertions/ResolvedLocalPairAssertions.w`, `compiler/syntax/booleans/BooleanTokens.w`, imported-constant `compiler/syntax/booleans/ResolvedBooleanLiteralComparisons.w`, imported-constant `compiler/syntax/comparisons/NamedComparisonKinds.w`, imported-constant `compiler/syntax/conditionals/LiteralComparisonOperations.w`, imported-constant `compiler/syntax/conditionals/NamedConditionalBases.w`, imported-constant `compiler/syntax/conditionals/NamedLiteralComparisonKinds.w`, imported-constant `compiler/syntax/conditionals/NamedLocalConditionalKinds.w`, imported-constant `compiler/syntax/conditionals/NamedLocalConditionalValues.w`, imported-constant `compiler/syntax/conditionals/ResolvedLiteralComparisonKinds.w`, imported-constant `compiler/syntax/conditionals/ResolvedLocalConditionalKinds.w`, imported-constant `compiler/syntax/conditionals/ResolvedLocalConditionalOperands.w`, imported-constant `compiler/syntax/conditionals/ResolvedLocalConditionalSources.w`, imported-constant `compiler/syntax/locals/NamedLongOperations.w`, imported-constant `compiler/syntax/locals/ResolvedLocalCopyKinds.w`, imported-constant `compiler/syntax/locals/ResolvedLocalEqualityKinds.w`, imported-constant `compiler/syntax/locals/ResolvedLocalInequalityKinds.w`, imported-constant `compiler/syntax/locals/ResolvedLocalLessThanKinds.w`, imported-constant `compiler/syntax/locals/ResolvedLocalLiteralComparisons.w`, imported-constant `compiler/syntax/locals/ResolvedLocalLiteralComparisonSources.w`, imported-constant `compiler/syntax/locals/ResolvedLongOperations.w`, imported-constant `compiler/syntax/loops/ResolvedLocalLoopForms.w`, imported-constant `compiler/syntax/loops/ResolvedLocalLoopKinds.w`, imported-constant `compiler/syntax/loops/ResolvedLocalLoopOperands.w`, imported-constant `compiler/syntax/updates/NamedLocalUpdateKinds.w`, imported-constant `compiler/syntax/updates/ResolvedLocalUpdates.w`, imported-constant `compiler/ir/OpcodeKinds.w`, imported-constant `compiler/ir/TypeKinds.w`, imported-constant `compiler/ir/InstructionForms.w`, imported-constant `compiler/syntax/BooleanDeclarationKinds.w`, `compiler/syntax/IdentifierStarts.w`, `compiler/syntax/tokens/CompilerTokenLimits.w`, `compiler/syntax/tokens/KeywordTokens.w`, `compiler/syntax/tokens/SourceScalars.w`, `compiler/syntax/helpers/HelperAbi.w`, imported-constant `compiler/syntax/helpers/HelperSignatures.w`, mixed-owner `compiler/syntax/helpers/HelperValueKinds.w`, imported-constant `compiler/syntax/EarlyReturnKinds.w`, imported-constant `compiler/syntax/EarlyReturnResultKinds.w`, `compiler/syntax/LoopKinds.w`, imported-constant `compiler/syntax/calls/CallArgumentSources.w`, imported-constant `compiler/syntax/calls/OneArgumentCalls.w`, imported-constant `compiler/syntax/calls/TwoArgumentCallKinds.w`, imported-constant `compiler/syntax/calls/FourArgumentCalls.w`, `compiler/syntax/calls/assignment/AssignmentCallArities.w`, `compiler/syntax/calls/assignment/AssignmentCallCodeWidths.w`, `compiler/syntax/calls/assignment/AssignmentCallColumns.w`, `compiler/syntax/calls/assignment/AssignmentCallIdentities.w`, `compiler/syntax/calls/assignment/AssignmentCallInstructionWidths.w`, `compiler/syntax/calls/assignment/AssignmentCallLocalWidths.w`, `compiler/syntax/calls/ThreeArgumentCalls.w`, `compiler/syntax/calls/VoidCallKinds.w`, `compiler/syntax/calls/VoidCallSourceKinds.w`, mixed-owner `compiler/syntax/calls/VoidCallSourceWidths.w`, imported-function `compiler/syntax/calls/void/VoidCallSourceForms.w`, `compiler/syntax/calls/VoidCallWidths.w`, imported-constant `compiler/syntax/returns/EarlyReturnSources.w`, imported-constant `compiler/syntax/returns/NamedBooleanReturnKinds.w`, imported-constant `compiler/syntax/returns/NamedReturnArithmeticKinds.w`, imported-constant `compiler/syntax/returns/NamedReturnComparisonOperands.w`, imported-constant `compiler/syntax/returns/NamedSignedReturnKinds.w`, imported-constant `compiler/syntax/returns/ResolvedEarlyComparisonKinds.w`, imported-constant `compiler/syntax/returns/ResolvedEarlyResultKinds.w`, imported-function `compiler/syntax/returns/EarlyComparisonForms.w`, `compiler/syntax/returns/ResolvedLocalReturns.w`, imported-constant `compiler/syntax/returns/ResolvedReturnCallKinds.w` modules. One through seven direct executable dependencies may jointly own one through twenty-two helpers while the root owns the remainder of the twenty-three-helper table. A twenty-third dependency helper, an eighth executable owner, and a twenty-fourth total helper still fail before publication. The linker orders up to seven direct helper owners by canonical root imports. All six three-owner orders plus forward/reverse rotations for four, five, six, and seven owners leave every function identity and byte in place. Signed-parameter Boolean and signed helpers accept bounded equality or less-than guards, computed signed-local preludes, and up to sixty-four same-module or direct imported Boolean calls with a typed literal, constant, or prior-local return before the final result. Sixty-four calls may span all twenty-two helpers from seven direct owners and reach the helper statement and 256-local boundaries. A sixty-fifth call is rejected before publication. A final Boolean return may forward one zero-, one-, or two-argument helper call. A Boolean helper-call guard may forward another one-argument helper result over the same or a different prior signed local. Thirty-two such pairs fill the sixty-four-call table. Equality and less-than guards may return any prior signed local. A signed less-than guard may instead return its parameter minus, divide by, or reduce modulo one literal or constant. Unresolved guard widths account for their generated temporaries before later declarations resolve, so a local after three guards does not read register four and call it destiny. The native header parser accepts up to sixty-four sorted unique direct import declarations. The bounded constant-import APIs execute every rooted acyclic scalar-constant graph from one through seven imported modules. The single-import path links directly. Complete graph plans own edge traversal, root rank, privacy, sharing, and source selection from two through seven imports. Topology identities and executor catalogues do not. Dense DAGs, redundant direct edges, and frame rotations match stage 0 byte for byte. Cycles, detached nodes, eight imports, non-ASCII linked source, and byte 32,769 fail before publication. Direct helper sets, mixed direct owners, redundant constant leaves, the constant-fed helper chain, private helper chains, and multi-input private helper dependencies use the graph executor. Mixed constant/helper inputs run through separate declaration and executable passes. Shared and redundant executable dependencies use exact owner-identity filters. Helper member groups remain once in canonical dependency order. General imported declarations and aggregate symbols remain unsupported.

The Wheeler-written compiler also lowers one scalar reversible helper. A `rev long`
helper with up to two signed parameters may return a signed literal, evaluated constant,
preserved signed parameter, checked operation over either signed parameter and a literal
or evaluated constant, or checked operation over two signed parameters. A `rev boolean`
helper may return a Boolean literal or one preserved Boolean parameter. Independent checked operations may bind signed locals before the tail return selects one exact relation. Its bounded entry interleaves result calls with signed equality checks against constants or
...
generated inverse, and optional proof certificate. Boolean arithmetic, three parameters,
chained preludes, mismatched local returns, and effectful helper statements fail before
publication.
...
The reversible result ABI accepts one `rev long` function with typed parameters and one
tail signed-literal, evaluated-constant, preserved-parameter, or checked
source-with-constant return. It also accepts `rev boolean` with a Boolean literal or one
preserved Boolean parameter. The caller creates an implicit vacant slot. Forward
execution fills it, and generated inverse execution checks the exact held constant,
source, or computed value before restoring vacancy. Six dedicated WIP-0038 instructions carry those
...
Allocation and access trap before mutation on invalid length, exhausted bytes or objects, bad handles, the wrong storage kind, invalid byte values, dropped owners, or an out-of-range index. An owned `setByte` advances the owner to the write's destination local. A following mutation or `drop` consumes that current local. The owner does not fork merely because the source spelling stays put. `freezeUtf8` consumes the current byte owner and returns one immutable UTF-8 owner. The recovery compiler currently admits that result on the two- and ten-parameter helper signatures required by bounded graph-source copying. An exact two-argument same-module call may forward the owner result when its primitive argument types match. The bounded graph-source profile also admits one exact owner-advancing UTF-8 copy loop and guarded selectors zero through six. It retains stage 0's trailing jump after each guarded return because canonical bytecode is allowed to be redundant but not imaginative. Other result signatures, loop bodies, and mismatched forwarding fail before publication.
