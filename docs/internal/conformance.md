...
- SHA evidence receives 1,000,000 startup transitions plus 200 transitions per input byte. The 137,159-byte reference compiler manifest hashes in 26,267,642 history-free transitions under its 28,431,800-transition budget. Manifest identity tests track current physical bytes separately. This checks linear work, not the patience of a progress bar.
