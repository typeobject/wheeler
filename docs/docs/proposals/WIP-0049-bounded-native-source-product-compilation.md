...
- [ ] Every physical compiler module publishes one source-local product artifact. The counted physical closure now compiles `wheeler.compiler.boolean_tokens`, `wheeler.compiler.identifier_starts`, `wheeler.compiler.resolved_local_returns`, `wheeler.compiler.void_call_kinds`, `wheeler.compiler.void_call_source_kinds`, and `wheeler.compiler.wide_return_sources` directly from immutable local archive ranges. Each artifact enters `CompiledBodyArchive.w` under its physical module owner before the ordered immutable archive matches stage 0 byte for byte. Closure-wide iteration remains.
