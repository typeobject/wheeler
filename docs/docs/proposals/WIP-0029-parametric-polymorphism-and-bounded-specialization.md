...
- hidden assumptions that `T` is copyable or droppable;
- unknown quantum shape during lowering;
...
- Which closed type-level intrinsics beyond WIP-0017 are required by first collections and quantum shapes (owner: language, library, and quantum maintainers; decision point: const-generic acceptance)?
