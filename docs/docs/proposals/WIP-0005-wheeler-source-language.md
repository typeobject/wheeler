...
- [ ] Deterministic classical function modules now enforce exact declarations, sorted direct imports, public visibility, dependency-first linking, and closed DAG inputs; manifest-bound source sets now build locally and from locked archives; direct public immutable-record and closed-variant APIs now link, and importers exhaustively match exported variants; nested/generic collections, exported state/proofs/circuits, cross-package linking, type qualification, and re-export remain; full module-qualified public function calls execute.
