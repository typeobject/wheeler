...
- signed `long`, `boolean`, immutable nominal records, closed tagged variants with exhaustive `match`, fixed immutable arrays with checked indexing, nonescaping immutable slices, and function-local affine `region`/`words`/`bytes`/`utf8`/`longmap` storage with bounded allocation, checked mutation, strict UTF-8 validation/counting, and explicit drop; typed left-to-right expressions and static calls; `return`, `if`/`else`, early return, bounded `while` and counted `for`, and `break`/`continue` in ordinary classical methods;
...
- [ ] Deterministic classical function modules now enforce exact declarations, sorted direct imports, public visibility, dependency-first linking, and closed DAG inputs; manifest-bound source sets now build locally and from locked archives; exported nominal types/state/proofs/circuits, cross-package linking, qualification, and re-export remain.
