...
- direct inverse calls, reverse blocks, and finite `inverse(function)`, `adjoint(circuit)`, and `equivalent(left, right)` theorem declarations;
...
The initial WIP-0011 slice resolves generated-inverse, generated-adjoint, and finite circuit-equivalence theorem declarations and emits canonical certificates checked by the finite kernel. Contracts, general propositions and terms, structured proof blocks, experiments, circuit/resource rules, and package proof APIs remain.
...
- [x] Classical state, signed/Boolean/record/variant/array values and nonescaping slice parameters/locals, bounded control flow, exhaustive selection, checked indexing, and reverse blocks lower to WIP-0001 major version 1 typed frames.
