...
- signed `long`, `boolean`, immutable nominal records, closed tagged variants with exhaustive `match`, fixed immutable arrays with checked indexing, nonescaping immutable slices, and function-local affine `region`/`words`/`bytes`/`utf8`/`longmap` storage with nonescaping immutable `utf8` parameters with bounded allocation, checked mutation, strict UTF-8 validation/counting, and explicit drop; typed left-to-right expressions and static calls; `return`, `if`/`else`, early return, bounded `while` and counted `for`, and `break`/`continue` in ordinary classical methods;
