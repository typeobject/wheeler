...
- [ ] WIP-0049 publishes one source-local artifact for every physical compiler module. Five physical scalar-dependent modules now pass the counted archive-to-artifact path byte for byte. Publication still needs to cover the remaining closure.
