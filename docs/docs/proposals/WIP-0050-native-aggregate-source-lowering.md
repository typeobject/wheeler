...
- [x] `compileAggregateSourceModuleProductWithImports` validates nominal scaffolding and projections, then compiles primitive body portions through nonretained signed carriers. It withholds nominal and exact function-local carrier projections until compilation succeeds. The carrier artifact contains no generated descriptor. Resolved aggregate instructions remain a separate semantic code product until source-local body archival admits their source-order splice points.
