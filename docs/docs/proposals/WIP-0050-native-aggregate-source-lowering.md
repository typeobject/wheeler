...
- [x] Primitive and recursive local nominal member types resolve before source release. `LocalNominalReferences.w` also indexes local nominal uses in callable signatures, parameters, bodies, and constructors while excluding aggregate declarations. `LocalNominalCarriers.w` rewrites those ranges to compact signed carriers and retains old-to-new coordinates. `LocalNominalCarrierProjections.w` separates value, constructor, and signature uses and binds value carriers to exact function-local coordinates. `CountedLocalNominalCarriers.w` carries value projections into closure coordinates for final local-type rewriting. `AggregateExpressionProjection.w` replaces outer aggregate expressions with offset-stable scalar placeholders before primitive compilation. Nested expressions share one placeholder statement. `AggregateFrontendBindings.w` derives each operation function from its exact destination value product. `AggregatePlaceholderPlacements.w` derives exact splice ordinals from compiled zero-local placeholders. `PrimitivePlaceholderProjection.w` then validates and removes those instructions before aggregate composition and adjusts function lengths and subsequent splice ordinals. `AggregateExpressionTemporaries.w` gives nested operations source-ordered locals while retaining each named outer destination. `SourceCallableTypeProducts.w` freezes primitive and local nominal signature types before temporary carrier source is discarded. `SourceStatementProducts.w` publishes callable-owned source ranges, named parameters, statement-result locals, and function local counts in the aggregate-aware compilation transaction. Nested aggregate temporaries are appended to the same value window. Local nominal carrier, destination, owner, argument, function, direction, and provisional placement coordinates are derived from those products rather than supplied by the caller. Local constructor descriptor and case targets are resolved before primitive artifact publication. Record and variant field owners are derived from callable-local nominal products, not supplied as fixture rows. The aggregate-aware compiler now joins those products into canonical operands and publishes an exact supplemental instruction product beside the primitive artifact. Decoded primitive products determine exact placeholder splice points. Assignment temporaries and their moves are removed as one validated bridge before selector composition. Primitive bytes, identities, metadata, supplemental bytes, and composition rows remain staged until every check passes. The complete source-product fixture exercises record and variant constructors plus record-field and variant-payload projections through this transaction. Indexed field chains resolve the producer member's structural target before fixed-array projection. Direct fixed-array parameters bind by exact canonical type spelling. Duplicate descriptors publish neither owner nor value products. Slice constructor destinations bind exact callable structural descriptors. Indexed slices use the same owner projection and select `SLICE_GET`. Missing or wrong-kind descriptors publish nothing. Duplicate cases and unresolved types publish nothing.
...
- [x] Source products cover nonescaping slice construction and indexing outside aggregate storage. Callable destination types select exact slice descriptors, while aggregate-stored slices remain rejected.
...
- [x] The native compiler derives aggregate locals from complete callable-owned frontend products and derives splice coordinates from decoded primitive placeholders as specified by WIP-0051.
