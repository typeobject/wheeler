...
- [x] `AggregateCodegen.w` emits canonical record, variant, fixed-array, and slice construction and projection forms. `AggregateInstructionProducts.w` validates a complete counted operation window and lowers it to canonical bytes. Native evidence covers every form and matches stage 0 for record construction and field projection. Invalid operands or unknown aggregate opcodes fail before a header byte is written.
