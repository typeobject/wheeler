...
- [x] `AggregateInstructionComposition.w` merges primitive and aggregate instructions at counted function, direction, and ordinal splice points. It publishes adjusted function lengths, source-artifact selectors, and one canonical instruction order only after both artifacts and every placement validate. `appendComposedFunctionProduct` carries the primitive and aggregate artifact ranks into closure instruction rows.
...
- [x] `compileAggregateSourceModuleProductWithImports` validates nominal scaffolding and projections, then compiles primitive body portions through nonretained signed carriers. It withholds nominal and exact function-local carrier projections until compilation succeeds. The carrier artifact contains no generated descriptor. Resolved aggregate instructions now compose with primitive instruction rows through source-artifact selectors, and counted closure rows retain both artifact ranks. Complete source-product compilation must derive local and splice products from the primitive frontend rather than fixture projections.
