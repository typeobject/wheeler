...
- [x] `compileAggregateSourceModuleProductWithImports` validates nominal scaffolding and projections, then compiles primitive body portions through nonretained signed carriers. It withholds projection publication until compilation succeeds. The carrier artifact contains no generated descriptor. Aggregate operations remain outside this path until native instruction lowering lands.
