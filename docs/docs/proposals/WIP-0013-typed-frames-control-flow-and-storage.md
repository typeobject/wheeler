...
- [ ] The Wheeler-written verifier/interpreter now differentially checks bounded owned-region/word/byte-buffer allocation, mutation, length, reads, byte ranges, strict UTF-8 validation/count/scalar/width behavior, deterministic fixed-capacity signed-map insert/update/lookup/membership, drop order, malformed index locals, and exact outer rewind. Frozen UTF-8 owners, borrows, cross-function ownership, and general native trace parity remain.
