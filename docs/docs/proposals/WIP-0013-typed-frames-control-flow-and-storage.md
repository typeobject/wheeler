...
- [ ] A bounded Wheeler scanner now consumes explicit host UTF-8 source input and emits identifier/number/punctuation/line-comment token kinds and starts into owned buffers; complete literals, diagnostics, parser, codec, verifier, and package graph fixtures remain.
