...
- [ ] The Wheeler-written verifier/interpreter now differentially checks bounded owned-region/word-buffer allocation, mutation, reads, drop order, malformed index locals, and exact outer rewind. Byte buffers, UTF-8, maps, borrows, cross-function ownership, and general native trace parity remain.
