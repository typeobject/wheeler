...
- [x] Local constants, state load/store, move, checked add/subtract/multiply, comparison, branches, loop checks, value calls, and value returns execute and rewind.
...
- [ ] Function-local regions now enforce byte/object ceilings, affine moves, leak-free exits, mutable signed-word/byte buffers, immutable validated UTF-8 owners with read-only parameter borrows, and region owners with exclusive scratch-allocation borrows and consumed empty-region results, word/byte/map owners with exclusive mutable borrows, explicit drop order, snapshots, and rewind; borrowing, typed collections, cross-function ownership, recoverable allocation, capabilities, and split/join borrowing, typed collections, general ownership transfer, recoverable allocation, capabilities, and compiler-scale arenas remain.
- [ ] Reversible protected control forms generate checked inverses.
- [ ] A bounded manifest-linked Wheeler scanner/parser pair now consumes explicit host UTF-8 source input and emits identifier/number/punctuation/line-comment token metadata with checked signed-decimal overflow into owned buffers and validates one assignment production through simultaneous exclusive buffer borrows and an exhaustive value/error result, then emits the parsed token through an exclusive external output borrow; complete literals, diagnostics, parser, codec, verifier, and package graph fixtures remain.
