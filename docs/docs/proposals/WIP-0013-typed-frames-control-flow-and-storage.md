...
- [ ] The Wheeler-written verifier/interpreter now differentially checks bounded owned-region/word/byte-buffer allocation, mutation, length, reads, byte ranges, drop order, malformed index locals, and exact outer rewind. UTF-8, maps, borrows, cross-function ownership, and general native trace parity remain.
