...
- [ ] Function-local regions now enforce byte/object ceilings, affine moves, leak-free exits, mutable signed-word/byte buffers, immutable validated UTF-8 owners with read-only parameter borrows, fixed-capacity signed maps with exclusive mutable parameter borrows, explicit drop order, snapshots, and rewind; borrowing, typed collections, cross-function ownership, recoverable allocation, capabilities, and compiler arenas remain.
...
- [ ] Region tests cover word/byte/map allocation and mutation, byte ranges, UTF-8 freeze/mutation/borrow/scalar boundaries, map borrow aliasing/capacity/absence, byte/object exhaustion, drop ordering, move/use-after-move, escape/leak rejection, ownership joins, canonical encoding, snapshots, and rewind; dangling borrows, output-address independence, recoverable failure, and commit remain.
