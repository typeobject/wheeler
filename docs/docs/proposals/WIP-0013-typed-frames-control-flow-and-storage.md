...
- [ ] Function-local regions now enforce byte/object ceilings, affine moves, leak-free exits, mutable signed-word/byte buffers, immutable validated UTF-8 owners with read-only parameter borrows, and word/byte/map owners with exclusive mutable parameter borrows, consumed empty-region results, explicit drop order, snapshots, and rewind; borrowing, typed collections, cross-function ownership, recoverable allocation, capabilities, and compiler arenas remain.
- [ ] Reversible protected control forms generate checked inverses.
- [ ] A bounded Wheeler immutable UTF-8 scanner now emits identifier/number/punctuation/line-comment token kinds and starts into owned buffers; complete literals, diagnostics, parser, codec, verifier, and package graph fixtures remain.
...
- [ ] Region tests cover word/byte/map allocation and mutation, byte ranges, UTF-8 freeze/mutation/borrow/scalar boundaries, buffer/map borrow kind/aliasing, map capacity/absence, byte/object exhaustion, drop ordering, move/use-after-move, escape/leak rejection, ownership joins, canonical encoding, snapshots, and rewind; dangling borrows, output-address independence, recoverable failure, and commit remain.
