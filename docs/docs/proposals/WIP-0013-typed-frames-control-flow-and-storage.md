...
- [ ] A bounded manifest-linked Wheeler scanner/parser pair now consumes explicit host UTF-8 source input and emits identifier/number/punctuation/line-comment token metadata into owned buffers and validates one assignment production through simultaneous exclusive buffer borrows and an exhaustive value/error result; complete literals, diagnostics, parser, codec, verifier, and package graph fixtures remain.
