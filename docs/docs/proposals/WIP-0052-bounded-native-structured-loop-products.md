...
- [x] `ResolvedLoopBodyProducts.w` publishes direct signed declaration, assignment, and checked local-update flow with exact monotonic local bases. Named sources require one visible prior value. Unsupported or ambiguous body rows publish nothing.
...
- [x] `LoopInstructionProducts.w` emits canonical forward instruction windows and exact branch targets for direct signed declarations, assignments, and checked local updates without mutating resolved products. The admitted fixture matches stage 0 byte for byte.
