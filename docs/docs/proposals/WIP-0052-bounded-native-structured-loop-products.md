...
`LoopBodyValues.w` owns unique visible-name and source-type joins against callable value products. `ResolvedLoopBodyProducts.w` uses those joins to resolve direct signed and Boolean-literal declarations, type-checked signed and Boolean assignments, Boolean and literal comparison assertions, checked local updates, and borrowed-word reads and writes from the block-grouped statement window. Named operands bind one visible callable value. Body scratch locals form one monotonic window. Later statements cannot reuse an earlier statement's temporaries. It records exact local bases, closed opcodes, operand forms, and operand values in one transaction. Unsupported statements and ambiguous names publish nothing. A child-block row also fails before body publication. `LoopNestedBlockProducts.w` owns the first nested slice: one-arm signed local equality or less-than guards over direct checked-add bodies. It validates the child link and complete body before publishing canonical comparison, branch, update, and join windows. `LoopCallProducts.w` emits validated zero- through seven-argument signed, Boolean, and void call windows with typed evaluation and transfer locals plus stable target-identity relocations. Each argument source must precede the call-local window and match the target's source-independent parameter type. `LoopBackEdgeProducts.w` consumes source-independent instruction windows, ownership state rows, and instruction-derived loan events. Entry and back-edge state must match exactly, and every loan acquired in the body must release before the jump.
...
- [x] `ResolvedLoopBodyProducts.w` publishes direct signed and Boolean-literal declaration, type-checked signed and Boolean assignment, Boolean and literal comparison assertion, checked local-update, and borrowed-word read/write flow with exact monotonic local bases. Named sources require one visible prior value. Unsupported, cross-type, or ambiguous body rows publish nothing.
- [ ] Resolved loop products join all admitted nested blocks. `LoopNestedBlockProducts.w` now publishes canonical one-arm signed local equality and less-than guards with direct checked-add children and rejects detached or recursively nested children atomically. `LoopCallProducts.w` publishes source-independent call code, types, and relocations, and `LoopBackEdgeProducts.w` validates ownership state and back edges.
