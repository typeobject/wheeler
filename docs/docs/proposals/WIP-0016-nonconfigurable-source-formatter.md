...
- [ ] Remaining syntax-owned break tables plus the minimal-diff generated corpus are accepted. Bounded loop headers, array initializers, and deeply indented indivisible tokens now have token-preserving, idempotent focused fixtures. The command is authoritative for its documented stage-0 subset instead of quietly claiming the rest.
