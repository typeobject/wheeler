...
- [x] `RecoveryDriverBootstrap` admits a bounded immutable input closure only when it contains the exact driver source identity, fits declared build-plan input and output ceilings, and names no network or `current-wheeler` command dependency. One explicit `PreviousDriver` boundary produces the first current driver. Output length and SHA-256 identity validate before the result publishes. Missing source and stale output fail before acceptance.
