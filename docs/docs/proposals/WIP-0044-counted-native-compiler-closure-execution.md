...
- [x] `ArchiveModuleSources.w` joins all 232 physical compiler modules to exact digest-matching archive ranges. A mismatched source identity leaves publication untouched.
...
- [x] WIP-0045 owns semantic module products. Scalar compilation transfers validated declarations and values instead of retaining linked source. The callable phase now publishes 1,001 physical compiler signature and body-range products. Typed signatures and compiled bodies remain.
...
- The current 232-module compiler closure plans without truncation.
