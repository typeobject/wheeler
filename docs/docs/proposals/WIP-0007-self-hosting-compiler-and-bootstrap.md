...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - Alternate names and numeric operands match stage 0 across empty and one- through sixteen-statement entry bodies. Helpers independently admit zero through sixteen statements. The tests cover signed and Boolean locals, assertions, one global, assignment, checked add or subtract, XOR, helper calls, reverse blocks, and an optional inverse theorem.
