...
- [x] Stage 0 accepts checked scalar `const` declarations with direct public import and canonical qualification and finite `enum` declarations elaborated to payload-free variants. Constants emit no globals; enum source reorder canonicalizes by case name. `compiler/Opcodes.w` now owns the Wheeler verifier/interpreter opcode identities and family predicates, replacing dispatch literals before the opcode surface widens. The constant dependency graph supports checked forward references and canonical cycle diagnostics; reversible finite permutations and coherent power-of-two enum lowering remain WIP-0017 work. At least the number soup now has labels on the tins.
- [x] `NativeVm.w` and `compiler/Interpreter.w` execute the verifier-approved bounded compiler artifact profile inside Wheeler and differentially agree with stage 0 for a checked local/global update plus the proof-bearing reversible `Counter.w`; malformed artifacts fail before interpretation and the outer execution rewinds exactly. General bytecode coverage and native code remain WIP-0008 work.
