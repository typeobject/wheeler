...
- [ ] The manifest-linked `BytecodeHeader.w`/`compiler/Encoding.w` slice emits its canonical fixed header, six-entry section directory, manifest, string table, scalar-global type table, empty variant table, and alignment padding with bounded little-endian integer writes and checked eight-byte offset alignment through explicit output, matches the stage-0 bytes, and rewinds exactly; general section-size and string-table derivation, remaining payload codecs, and complete artifact validation remain.
- [ ] `SeedArtifact.w` emits a complete 360-byte canonical `.wbc` for a fixed minimal `Seed` program; every byte matches stage 0, strict decoding and canonical re-encoding succeed, and both the direct VM path and `wheeler run` atomic-output path execute the emitted artifact after a 512-byte capacity is trimmed to the canonical 360-byte length by Wheeler code. General IR-driven section sizing and payload emission remain. The seed is a writer conformance point, not a compiler wearing a fake moustache.
- [ ] Wheeler `.wbc` codec passes stage-0 differential tests.
- [ ] The manifest-bound `Utf8Lexer.w`/`lexer/Parser.w`/`lexer/Scanner.w` module graph consumes explicit bounded host source input and executes an immutable UTF-8 scanner for identifiers, signed decimal values through `Long.MAX_VALUE` with recoverable overflow, punctuation, whitespace, printable raw-ASCII literals, and terminated line/block comments with explicit token kind/start/length buffers and checks one `long` local-declaration production through a dependency parser module with an exported closed value/error result, then emits the accepted decimal token through explicit bounded output with a checked publish length; complete token schemas, escaped source literals, structured diagnostics, streaming/multiple effects, full parser construction, recovery sets, and corpus parity remain.
