...
- [x] Each CI producer emits the same canonical verified artifact-set manifest before comparison; hosted Temurin/Zulu run `29669820394` verified and byte-compared the complete trees for `5afa1b4`.
...
- [ ] Signed/Boolean parameters and results, typed frames, static calls, aggregate values, bounded control, affine local regions, word/byte buffers, immutable validated UTF-8 owners, and fixed signed maps, nested immutable UTF-8 borrows, and exclusive region/word/byte/map borrows and empty-region ownership results execute; WIP-0028 general cross-function ownership/loans and compiler-scale arenas, WIP-0029 generic collections, WIP-0030 static class evidence, and WIP-0031 typed callables/effects remain, along with library strings, exported collection APIs, and package aliases.
