...
- [ ] The manifest-linked `BytecodeHeader.w`/`compiler/Encoding.w` slice emits its canonical 40-byte fixed `.wbc` header and six-entry section directory with bounded little-endian integer writes through explicit output, matches the stage-0 bytes, and rewinds exactly; general sizing/layout passes, section payload codecs, and complete artifact validation remain.
