...
- [ ] The manifest-linked `MinimalCompiler.w` scanner/parser/encoding graph accepts a bounded minimal Wheeler source file, derives and canonically sorts source identifier strings, computes aligned section sizes, and emits a complete canonical minimal `.wbc`; for `LongClass` with `state long value = 7` and `value += 5`, every one of 504 bytes matches stage 0, strict decoding and canonical re-encoding succeed, and both direct VM and `wheeler run` paths execute the artifact after Wheeler trims a 512-byte capacity. Alternate class/global names, numeric operands, and empty entries, plain assignments, and checked add/subtract/XOR updates match stage 0 with IR-derived zero-, one-, or two-local descriptors and code lengths, while duplicate string identities fail before publication. General function/type/control-flow IR remains; signed global IR and assignment and checked arithmetic-update code emission have crossed the line. The accepted grammar is still tiny, but this is now a compiler slice rather than a writer wearing a fake moustache.
