...
- [ ] The manifest-linked `MinimalCompiler.w` scanner/parser/encoding graph accepts a bounded minimal Wheeler source file, derives string and aligned section sizes, and emits a complete canonical minimal `.wbc`; for `LongClass` with `state long value = 7` and `value += 5`, every one of 504 bytes matches stage 0, strict decoding and canonical re-encoding succeed, and both direct VM and `wheeler run` paths execute the artifact after Wheeler trims a 512-byte capacity. General function/type/control-flow IR remains; signed global IR and checked update code emission have crossed the line. The accepted grammar is still tiny, but this is now a compiler slice rather than a writer wearing a fake moustache.
