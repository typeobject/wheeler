...
- [ ] The manifest-linked `BytecodeHeader.w`/`compiler/Encoding.w` slice emits its canonical fixed header, six-entry section directory, and 24-byte manifest section with bounded little-endian integer writes through explicit output, matches the stage-0 bytes, and rewinds exactly; general sizing/layout passes, remaining section payload codecs, and complete artifact validation remain.
