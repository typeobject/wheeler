...
- [ ] The manifest-linked `BytecodeHeader.w`/`compiler/Encoding.w` slice emits its canonical fixed header, six-entry section directory, manifest, string table, and alignment padding with bounded little-endian integer writes and checked eight-byte offset alignment through explicit output, matches the stage-0 bytes, and rewinds exactly; general section-size and string-table derivation, remaining payload codecs, and complete artifact validation remain.
