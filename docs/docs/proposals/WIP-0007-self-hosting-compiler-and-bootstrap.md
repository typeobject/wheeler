...
- [x] The accepted Wheeler compiler, verifier, interpreter, scanner, and binary-codec sources live only in the canonical `wheeler.compiler` package. Its `compiler` tool and entryless `library` targets compile from exact manifest source sets; the examples consume an exact locked and vendored archive, and the workspace, formatter, documentation checker, Tree-sitter corpus, and differential fixtures follow the canonical source root. There is no example-side compiler copy available for archaeology or accidental linkage.
