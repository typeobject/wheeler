...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - Alternate names and numeric operands match stage 0 across empty and one- through eight-statement entry bodies. The tests cover signed and Boolean locals, assertions, one global, assignment, checked add or subtract, XOR, helper calls, reverse blocks, and an optional inverse theorem.
  ...
  - Fixed signed/Boolean arrays may now live in record fields and variant payloads. `MinimalProgram` uses eight-slot opcode and operand columns instead of ordinal fields. One shared sequence resolver owns local-name lookup and duplicate rejection; bounded loops derive local windows, code lengths, type rows, forward helper bodies, and reverse-order inverse bodies. Adding the eighth statement changed a bound, not the constitution.
