...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - Alternate names and numeric operands match stage 0 across empty and one- through thirty-two-statement entry bodies. Public, explicit-private, and unqualified helpers independently admit zero through thirty-two statements. Ordinary helpers work with or without class state. Empty reversible helpers and their generated-inverse theorems also need no dummy global; nonempty reversible helpers retain the bounded state-update contract. Duplicate visibility is rejected instead of being treated as emphasis. Canonical `module a.b;` headers produce the same qualified `a.b::main` and helper strings as stage 0 while retaining unqualified theorem names; malformed or noncanonical headers fail before output. Imports and multi-file linking remain outside this bounded slice. The tests cover signed and Boolean locals, assertions, one global, assignment, checked add or subtract, XOR, helper calls, reverse blocks, and an optional inverse theorem.
  ...
  - Fixed signed/Boolean arrays may now live in record fields and variant payloads. `MinimalProgram` uses thirty-two-slot opcode and operand columns instead of ordinal fields. `BodyParser.w` scans entry and helper bodies once into a caller-owned start table; the two grammar owners no longer carry matching hand-unrolled ladders. One shared sequence resolver owns local-name lookup and duplicate rejection; bounded loops derive local windows, code lengths, type rows, forward helper bodies, and reverse-order inverse bodies. The thirty-third statement fails before publication. Empty columns come from one IR owner rather than four copied literals; if raising a table bound requires another thicket of `if` statements, the table has already won.
