...
- [ ] The manifest-linked `BytecodeHeader.w`/`compiler/Encoding.w` slice emits the canonical 24-byte fixed `.wbc` header prefix with bounded little-endian integer writes through explicit output and exact rewind; section sizing, directory construction, payload codecs, and complete artifact validation remain.
