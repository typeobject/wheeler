...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - `StringTable.w` owns canonical string planning and emission. Its three-way stateless-helper plan compares the fully qualified helper and entry strings rather than comparing an unqualified apple with a qualified ioctl. Module and nonmodule artifacts match stage 0. `Driver.w` coordinates sections and returns exact verified artifact/code bounds. `MinimalCompiler.w` only publishes that range for the package tool. `SeedWriterProject` now owns one ordered integration-fixture source list for both manifest emission and physical copying, so tests cannot forget half of a module addition.
