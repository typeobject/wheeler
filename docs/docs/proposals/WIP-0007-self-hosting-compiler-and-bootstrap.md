...
- [ ] The manifest-bound `Utf8Lexer.w`/`lexer/Scanner.w` module pair consumes explicit bounded host source input and executes an immutable UTF-8 scanner for identifiers, signed decimal values through `Long.MAX_VALUE` with recoverable overflow, punctuation, whitespace, and terminated line/block comments with explicit token kind/start/length buffers and checks one assignment production through a borrowed-buffer parser helper with a closed value/error result, then emits the accepted decimal token through explicit fixed output; complete token schemas, literals/escapes, structured diagnostics, streaming/multiple effects and variable-length output protocols, full parser construction, a packaged parser-result API, and corpus parity remain.
