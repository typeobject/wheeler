...
- [ ] `Utf8Lexer.w` executes a bounded immutable UTF-8 scanner for identifiers, checked decimal values, punctuation, whitespace, and line comments with explicit token kind/start/length buffers; complete token schemas, literals/escapes/block comments, diagnostics, source input capabilities, parser construction, and corpus parity remain.
