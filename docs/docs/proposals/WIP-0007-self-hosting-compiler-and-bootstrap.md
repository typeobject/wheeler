...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - `StringTable.w` owns canonical string planning and emission. `Driver.w` coordinates sections and returns exact verified artifact/code bounds; `MinimalCompiler.w` only publishes that range for the package tool.
