...
- [ ] The manifest-linked `SeedArtifact.w`/`compiler/Encoding.w` writer accepts a bounded ASCII class name, derives string and aligned section sizes, and emits a complete canonical minimal `.wbc`; for `LongClass`, every one of 368 bytes matches stage 0, strict decoding and canonical re-encoding succeed, and both direct VM and `wheeler run` paths execute the artifact after Wheeler trims a 512-byte capacity. General function/global/type IR and code emission remain. The seed is a writer conformance point, not a compiler wearing a fake moustache.
