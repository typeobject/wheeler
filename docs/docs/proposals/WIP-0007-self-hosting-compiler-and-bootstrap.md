...
- [ ] `Utf8Lexer.w` consumes explicit bounded host source input and executes an immutable UTF-8 scanner for identifiers, checked decimal values, punctuation, whitespace, and line comments with explicit token kind/start/length buffers; complete token schemas, literals/escapes/block comments, diagnostics, streaming/multiple input and output capabilities, full parser construction, and corpus parity remain.
