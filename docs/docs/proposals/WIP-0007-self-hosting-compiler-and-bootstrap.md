...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - Alternate names and numeric operands match stage 0 across empty and one- through sixteen-statement entry bodies. Helpers independently admit zero through sixteen statements. Canonical `module a.b;` headers now produce the same qualified `a.b::main` and helper strings as stage 0; malformed or noncanonical headers fail before output. Module proofs and imports remain outside this bounded slice. The tests cover signed and Boolean locals, assertions, one global, assignment, checked add or subtract, XOR, helper calls, reverse blocks, and an optional inverse theorem.
