...
- [ ] The manifest-bound `Utf8Lexer.w`/`lexer/Scanner.w` module pair consumes explicit bounded host source input and executes an immutable UTF-8 scanner for identifiers, checked decimal values, punctuation, whitespace, and line comments with explicit token kind/start/length buffers and checks one assignment production through a borrowed-buffer parser helper with a closed value/error result; complete token schemas, literals/escapes/block comments, diagnostics, streaming/multiple input and output capabilities, full parser construction, exported variant APIs, and corpus parity remain.
- [x] A classical entry may receive one explicit strict UTF-8 input borrow; VM/runtime APIs and `wheeler run --input` enforce a 16 MiB bound, physical nonsymlink CLI paths, no ambient lookup, and exact rewind to the external-input baseline.
