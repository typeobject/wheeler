...
- [ ] The bounded Wheeler compiler slice now covers these cases:
  ...
  - Fixed signed/Boolean arrays may now live in record fields and variant payloads. `MinimalProgram` uses fixed opcode and operand columns instead of ten ordinal fields, and bounded loops derive local windows, code lengths, type rows, forward helper bodies, and reverse-order inverse bodies. The fifth statement is now data, not a small constitutional amendment.
