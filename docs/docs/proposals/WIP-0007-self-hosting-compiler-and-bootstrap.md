...
- [ ] The manifest-linked `MinimalCompiler.w`/`compiler/{Encoding,HelperParser,Ir,Parser,Statements,Structure,Tokens,Verifier}.w`/scanner graph accepts a bounded minimal Wheeler source file, derives and canonically sorts source identifier strings, computes aligned section sizes, and emits a complete canonical minimal `.wbc`; for `LongClass` with `state long value = 7` and `value += 5`, every one of 504 bytes matches stage 0, strict decoding and canonical re-encoding succeed, and both direct VM and `wheeler run` paths execute the artifact after Wheeler trims a 512-byte capacity. Alternate class/global names, numeric operands, and no-global empty/signed-local entries, one-global empty entries, and one- through four-statement local/assignment/checked add/subtract/XOR/assertion bodies and a named one-statement helper plus static entry call or reversible helper plus reverse block and optional generated-inverse theorem match stage 0 with IR-derived zero through eight local slots and code lengths; two-function artifacts derive four-way string ordering, descriptor/type offsets, `RETURN`, and `CALL`; reversible helpers additionally derive intrinsic inverse code, inverse ranges, and `UNCALL`; certified forms derive a fifth canonical string, seventh directory entry, and `GENERATED_INVERSE` proof payload accepted by the kernel; signed local declarations compose with following state operations and lower through `LOCAL_CONST`/`LOCAL_MOVE` with derived local bases, global assertions lower to local-free `EXPECT_EQ`, signed initializers/operands use checked two's-complement emission and overlong magnitudes fail before output, while duplicate string identities fail before publication. A Wheeler-native bounded verifier rereads the private output and checks header, directory, payload, descriptor, code-length, and terminal-`HALT` invariants before `setOutputLength`; malformed bytes are rejected by a direct Wheeler negative test. General multi-function/local/type/control-flow IR remains; optional signed-global IR and assignment and checked arithmetic-update code emission have crossed the line. The accepted grammar is still tiny, but this is now a compiler slice rather than a writer wearing a fake moustache.
