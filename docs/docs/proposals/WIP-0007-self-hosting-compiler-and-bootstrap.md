...
- [ ] `SeedArtifact.w` emits a complete 360-byte canonical `.wbc` for a fixed minimal `Seed` program; every byte matches stage 0, strict decoding and canonical re-encoding succeed, and both the direct VM path and `wheeler run` atomic-output path execute the emitted artifact. General IR-driven section sizing and payload emission remain. The seed is a writer conformance point, not a compiler wearing a fake moustache.
- [ ] Wheeler `.wbc` codec passes stage-0 differential tests.
- [ ] The manifest-bound `Utf8Lexer.w`/`lexer/Parser.w`/`lexer/Scanner.w` module graph consumes explicit bounded host source input and executes an immutable UTF-8 scanner for identifiers, signed decimal values through `Long.MAX_VALUE` with recoverable overflow, punctuation, whitespace, printable raw-ASCII literals, and terminated line/block comments with explicit token kind/start/length buffers and checks one `long` local-declaration production through a dependency parser module with an exported closed value/error result, then emits the accepted decimal token through explicit fixed output; complete token schemas, escaped source literals, structured diagnostics, streaming/multiple effects and variable-length output protocols, full parser construction, recovery sets, and corpus parity remain.
  EOF closes neither a block comment nor a literal. Waiting longer has not made that a sound recovery rule.
- [x] A classical entry may receive explicit strict UTF-8 input and fixed mutable byte-output borrows; VM/runtime APIs and `wheeler run` effect options enforce 16 MiB bounds, physical nonsymlink input, atomic output publication after success, no ambient lookup, and exact rewind to the external-effect baseline.
