...
- [ ] `SeedArtifact.w` emits a complete 360-byte canonical `.wbc` for a fixed minimal `Seed` program; every byte matches stage 0, strict decoding and canonical re-encoding succeed, and the emitted artifact executes. General IR-driven section sizing and payload emission remain. The seed is a writer conformance point, not a compiler wearing a fake moustache.
