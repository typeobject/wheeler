...
- [x] `NativeVm.w` and `compiler/Interpreter.w` execute the verified bounded compiler profile inside Wheeler: signed global initialization, local constants/load/store/move/arithmetic, expectations, direct reversible global operations, bounded zero-argument frames, `CALL`, `UNCALL`, `RETURN`, and `HALT`. A checked update agrees with the stage-0 VM and rewinds the outer interpreter exactly; the proof-bearing `Counter.w` artifact emitted by the Wheeler-written compiler matches stage 0 byte-for-byte and returns to zero. General control flow, aggregate/storage opcodes, argument calls, interpreter-level rewind records, workflows, and native machine code remain. The runtime has entered the building, but it has not yet found every light switch.
