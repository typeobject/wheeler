...
- [x] The finite kernel checks exact QFT adjoint involution and same-register adjacent-inverse rewrite certificates. It rejects a nonreversible subject, invalid rule arguments, and forged inverse bodies. Semantic composition, general rewrite equivalence, scalar normalization, and global-phase rules remain.
...
- [x] Runtime assertion reduction gives proof, exact-quantum, and sampled-quantum evidence distinct closed kinds. Sampled evidence cannot be constructed as proof evidence or converted by payload coincidence. Only an explicit content-identified sampled comparison can resolve `Inconclusive`.
