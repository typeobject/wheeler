...
- [x] A Wheeler-written conformance fixture emits every byte of a minimal canonical artifact from a bounded input class name and derived aligned layout; stage 0 agrees byte-for-byte, the strict decoder accepts it, canonical re-encoding is identical, and the VM executes it.
