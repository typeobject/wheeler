...
- [ ] Stage-0 manifests, resolution, lockfiles, build plans, and archives are content-addressed and reproducible; exact offline dependency targets now build in dependency-first order; exact manifest-bound module source sets with direct public function/record/closed-variant/scalar-array/slice APIs and imported exhaustive matches now parse and link dependency-first in local, workspace, planned, archived, and locked offline builds, while cross-package dependency APIs and native build execution remain.
