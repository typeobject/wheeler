...
- [ ] Snapshot identity in locks, build-input-keyed build-output reuse, quarantine/GC, mirrors, and signed network snapshots remain.
...
- [ ] Vendor retries and relocation preserve exact locked bytes and reject poisoned files; XDG repository paths are identity-neutral, and package-object cache poisoning/deletion are tested, while build-output cache poisoning and identity-preserving mirrors remain.
