...
- [x] `NativePlan.w` and `packages/Plan.w` consume canonical binary plan bytes, verify exact framing/schema/payload length and Wheeler-computed payload SHA-256, decode one no-input/no-capability node, validate bounded ASCII names, numeric three-part release, output path, target kind, all five execution limits, and the node identity rederived from the exact length-prefixed domain/name/version/hex-identity/kind/path/count/limit fields, rewind exactly, and reject payload/digest corruption plus a re-signed invalid kind or forged node identity. Multiple nodes, package inputs, capabilities, Unicode strings, prereleases, and canonical re-encoding remain. A payload digest catches rot; it does not deputize every field as honest.
