...
- [x] The root `wheeler.workspace`, canonical core, compiler, and package-codec packages, and example package form an executable stage-0 workspace. The examples bind the exact compiler archive through their committed lock and vendor tree rather than a source-directory side door.
