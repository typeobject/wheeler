...
- [x] Lock schema 3 binds every selected package to both the stable repository identity that owned its candidate set and the complete canonical snapshot view used for selection; aliases, URLs, list positions, and physical paths remain outside lock identity. Existing exact selections remain preferred when still admissible, but a newly written lock records the current snapshot even when an unrelated release moved it. Sealed explicit catalogs use one domain-separated bootstrap repository identity and a content-derived snapshot rather than laundering either path into the graph.
...
- [x] Stage-0 plan execution derives `wheeler-build-input-1` from workspace, compiler, profile, and the complete canonical node; reuses output only when every planned node has an exact verified schema-1 input/PREV/length record; independently decodes each cached `.wbc`; and rebuilds the whole closed tree on any miss. A different verified PREV for one input is retained only under deterministic XDG quarantine state and aborts publication. Bounded `wheeler cache gc` removes malformed/unreferenced build records and objects without following links. Cache corruption, deletion, and divergent output cannot become resolver or build authority. The current identity covers source-package plan inputs; WIP-0023 recipe RREV/variant and native-tool axes remain.
...
- [x] `NativeWorkspace.w` and `packages/workspace/Workspace.w` parse bounded canonical-YAML workspaces into caller-owned member name/path range tables rather than a record with numbered upholstery. They check schema, keys, names, paths, lexical name order, path uniqueness, and pairwise nonnesting before exact byte publication through shared `LineEmitter.w`. Five ordinary members and a generated sixteen-member capacity boundary pass the independent stage-0 parser; a seventeenth member, wrong schema or key, malformed name, duplicate or unsorted name, duplicate or nested path, and traversal fail before publication; rewind restores the input-effect baseline. The native recovery scanner still bounds input to 4,096 bytes and the parser loop to 512 members, below the stage-0 schema's ten-thousand-member ceiling.
- [x] `NativeLock.w`, `packages/resolution/Lock.w`, and shared `LineEmitter.w` parse canonical schema-3 YAML into caller-owned package columns, dependency windows, and flat edge-target tables. Empty, two-package forward-edge, and generated six-package locks publish exact validated bytes, rewind, and pass the independent stage-0 parser; a seventh package exhausts the fixture table before publication. The parser checks sorted unique package and dependency names, valid releases, lowercase 64-nybble root/repository/snapshot/archive/manifest identities, and every target against the complete package set; wrong schemas, uppercase digests, duplicate or unsorted names, and unknown targets fail closed. The smaller executable fixture stays under retained-history limits; native package/edge loops remain capped at 512/1,024 under the 4,096-byte recovery scanner, below schema 3's ten-thousand-package ceiling. Dependency resolution has not found inner peace, but the lock model has at least stopped numbering the chairs.
- [x] `NativeManifest.w` and the `packages` modules scan explicit canonical YAML in Wheeler and parse variable target, source-selector, dependency, and capability collections into caller-owned fixed-width row tables plus quote-free header ranges. Every modular target may own a sorted nonempty selector list that covers its root; target/dependency names and capability tuples are sorted and unique; empty dependency/capability sections work; target/dependency kinds, tests, names, modules, paths, releases, and constraints are closed and checked. The ordinary two-target fixture and a generated eight-target/empty-section manifest publish exact bytes, rewind, and pass the independent stage-0 parser; a ninth target exhausts the fixture table before publication. Wrong schemas/kinds, test-selected libraries, malformed names/paths, unsorted selectors, and missing root coverage fail closed. Native loops remain capped at 512 collection rows and 1,024 selectors under the 4,096-byte recovery scanner, below the stage-0 schema ceilings; build metadata, owned decoded values, and computed manifest identity remain. A bounded differential fixture is evidence, not a package manager wearing a sash.
- [x] The unified stage-0 `wheeler` command checks, builds, executes test-selected runnable targets, and safely cleans canonical local workspaces; packages and verifies local artifacts; resolves explicit verified archive catalogs, materializes exact offline vendor trees, manages ordered XDG repository policy, publishes/fetches immutable canonical-YAML local releases; emits, verifies, and executes source-bound plans with exact package inputs, bounded output/time policy, complete request-scoped grants, exact artifact sets, and atomic publication; verifies locks; and compiles, runs, disassembles, and emits OpenQASM.
...
- [x] Resolution is identical under catalog and manifest insertion order with bounded backtracking; configured repository order is honored exactly, the first authoritative admissible repository wins without cross-repository version mixing, and filesystem enumeration order is sorted before candidates reach the solver.
