...
- [x] The stage-0 in-memory resolver deterministically selects one version per package with bounded backtracking, explicit development scope, cycle rejection, and stable ranges that ignore prerelease candidates unless the requirement names one.
