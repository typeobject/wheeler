...
- [x] `NativeManifest.w` and `packages/Manifest.w` scan explicit UTF-8 input in Wheeler and parse the mandatory package/name/version/profile header and one `target example NAME root PATH;` declaration and one `dependency runtime NAME version CONSTRAINT;` edge, and one `capability NAME path PATH;` request into typed source ranges, validates three-part releases with SemVer prerelease identifiers and exact/caret/tilde constraint syntax with checked 64-bit components, and rewinds exactly and fail-closed keyword rejection. Other target/dependency kinds, module/source lists, build metadata, other capability kinds, escape decoding, duplicate checks, and canonical re-emission remain; nine strings do not a package manager make, despite what marketing may have been told.
