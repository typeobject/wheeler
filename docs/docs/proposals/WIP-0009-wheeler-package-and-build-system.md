...
- [x] `NativeManifest.w` and the `packages` modules scan explicit canonical YAML in Wheeler and parse schema/key structure, one or two targets, an optional module with one to four sources, one or two dependencies, and one or two capabilities into typed source ranges. They validate the closed target/dependency kinds, booleans, names, modules, paths, releases, and constraints, publish the exact validated bytes, and rewind. Empty sections, general list counts, complete duplicate/order checks, logical patterns, build metadata, and owned decoded field values remain. A bounded differential fixture is evidence, not a package manager wearing a sash.
