...
- [x] `NativeLock.w`, `packages/resolution/Lock.w`, and shared `LineEmitter.w` parse canonical schema-2 YAML into caller-owned package columns, dependency windows, and flat edge-target tables. Empty, two-package forward-edge, and generated eight-package locks publish exact validated bytes, rewind, and pass the independent stage-0 parser; a ninth package exhausts the fixture table before publication. The parser checks sorted unique package and dependency names, valid releases, lowercase 64-nybble root/repository/archive/manifest identities, and every target against the complete package set; wrong schemas, uppercase digests, duplicate or unsorted names, and unknown targets fail closed. Native package/edge loops remain capped at 512/1,024 under the 4,096-byte recovery scanner, below schema 2's ten-thousand-package ceiling. Dependency resolution has not found inner peace, but the lock model has at least stopped numbering the chairs.
