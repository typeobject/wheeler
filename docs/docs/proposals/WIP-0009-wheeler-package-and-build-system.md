...
- [x] The stage-0 in-memory resolver deterministically selects one version per package with bounded backtracking, exact root/dependency source-profile matching, explicit development scope, cycle rejection, and stable ranges that ignore prerelease candidates unless the requirement names one.
