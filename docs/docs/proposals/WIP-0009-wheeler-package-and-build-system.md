...
- [x] `NativeManifest.w` and `packages/Manifest.w` scan explicit UTF-8 input in Wheeler and parse the mandatory package/name/version/profile header and one `target example NAME root PATH;` declaration into typed source ranges with exact rewind and fail-closed keyword rejection. Other target kinds, module/source lists, dependencies, capabilities, escape decoding, duplicate checks, and canonical re-emission remain; five strings do not a package manager make, despite what marketing may have been told.
