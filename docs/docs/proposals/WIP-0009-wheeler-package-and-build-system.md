...
- [x] The root `wheeler.workspace`, canonical compiler and package-codec packages, and example package form an executable stage-0 workspace. The examples bind the exact compiler archive through their committed lock and vendor tree rather than a source-directory side door.
...
- [x] Canonical `wheeler.packages.binary` owns bounded little-endian reads and ASCII name/release/path checks for binary package codecs; `Plan.w` no longer carries a private copy of the rules. A helper module is cheaper than two subtly different security boundaries.
...
- [x] The bounded manifest, lock, workspace, plan, archive, canonical-line, name/path/version, and SHA-256 modules are promoted into the entryless `wheeler.packages` library. That package locks its scanner/binary dependency to `wheeler.compiler`; examples lock both archives and retain only executable roots.
