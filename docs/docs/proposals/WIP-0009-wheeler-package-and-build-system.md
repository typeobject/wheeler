...
- [x] `NativeWorkspace.w` and `packages/workspace/Workspace.w` parse the bounded canonical-YAML recovery fixture with exactly two sorted members, checked schema/keys/names/paths, distinct nonnested roots, exact byte publication through shared `LineEmitter.w`, rewind, and stage-0 parser acceptance. General member counts remain; two directories are a workspace, while one directory is merely confident.
