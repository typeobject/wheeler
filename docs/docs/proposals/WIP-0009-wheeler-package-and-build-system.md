...
- [x] `NativeManifest.w` and `packages/Manifest.w` scan explicit UTF-8 input in Wheeler and parse the mandatory package/name/version/profile header and one `target example NAME root PATH;` declaration and one `dependency runtime NAME version CONSTRAINT;` edge, and one `capability NAME path PATH;` request into typed source ranges with exact rewind and fail-closed keyword rejection. Other target/dependency kinds, module/source lists, other capability kinds, escape decoding, duplicate checks, and canonical re-emission remain; nine strings do not a package manager make, despite what marketing may have been told.
