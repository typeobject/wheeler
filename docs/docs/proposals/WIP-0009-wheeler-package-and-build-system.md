...
- [x] `NativeLock.w`, `packages/resolution/Lock.w`, and shared `LineEmitter.w` parse the canonical-YAML two-package recovery lock, lowercase 64-nybble identities, and one known dependency; publish the exact validated bytes, rewind, and reject wrong schemas, uppercase digests, duplicate packages, and unknown/reversed dependencies. General graphs remain. The lock is small because the fixture is small, not because dependency resolution has discovered inner peace.
