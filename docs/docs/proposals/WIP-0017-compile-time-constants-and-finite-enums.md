...
- [x] The Wheeler-written recovery compiler validates a prefix of up to sixty-four literal scalar constants and substitutes them into typed locals, direct helper returns, scalar assignments, checked signed updates, and one- or two-argument scalar helper calls. Calls and mutations may mix constants and prior locals. Helper parameters and locals cannot shadow constants. Duplicate names, type mismatches, malformed declarations, unsupported expressions, and excess declarations publish nothing. Differential fixtures prove byte parity, no hidden globals, declaration-order independence, and typed signed and Boolean call parity for this native slice.
