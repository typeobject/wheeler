...
- [x] The accepted bounded signed FIFO, signed-map operation, fixed-array/slice reduction, and immutable UTF-8 inspection APIs live in the entryless `wheeler.core` package, build from an exact source set, and are consumed by the example package through its committed lock and vendor archive.
