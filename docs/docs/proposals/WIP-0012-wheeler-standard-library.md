...
- [ ] Scalar/aggregate values, affine byte buffers, strict UTF-8 freezing/scalar decoding with nested read-only parameter borrowing, and borrowed region scratch allocation plus mutable word/byte buffers and fixed signed maps with checked exclusive nested borrowing execute as language/VM substrate; explicit strict host UTF-8 or immutable arbitrary-binary `byteview` input and bounded byte-output entry borrows with a checked rewindable publish length now execute below the library, a Wheeler module performs bounded nonnegative little-endian integer emission and checked eight-byte layout alignment for the bytecode-writer slice, `crypto/Sha256.w` computes provider-free SHA-256 across empty/text/binary and one-/two-block padding fixtures using canonical checked `LOCAL_AND`/`LOCAL_ROTR32` plus explicit region scratch and supports checked subrange hashing for framed codecs, and the canonical entryless `wheeler.core` package exports a signed FIFO using an exclusive word-buffer borrow, immutable cursor, and explicit `Full`/`Empty` variants plus deterministic insert/lookup and membership operations over an exclusive signed-map borrow and fixed four-array/two-slice reductions. `WorkQueue.w` and `LongMap.w` consume that exact locked archive rather than an example-side copy. Wheeler generic option/result, library `String`, normalization, generic maps/sets/owned queues, iteration, canonical collection encoding, decode errors, and the broader package layers remain.
- [x] The accepted bounded signed FIFO, signed-map operation, and fixed-array/slice reduction APIs live in the entryless `wheeler.core` package, build from an exact source set, and are consumed by the example package through its committed lock and vendor archive.
