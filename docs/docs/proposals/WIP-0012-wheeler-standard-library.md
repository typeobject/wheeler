...
- [ ] Scalar/aggregate values, affine byte buffers, strict UTF-8 freezing/scalar decoding with nested read-only parameter borrowing, and fixed signed maps with checked exclusive nested borrowing execute as language/VM substrate; Wheeler option/result, library `String`, normalization, generic maps/sets/queues, iteration, canonical collection encoding, and packaged modules remain.
