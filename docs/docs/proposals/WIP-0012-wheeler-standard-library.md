...
- [ ] Scalar/aggregate values, affine byte buffers, strict UTF-8 freezing/scalar decoding with nested read-only parameter borrowing, and borrowed region scratch allocation plus mutable word/byte buffers and fixed signed maps with checked exclusive nested borrowing execute as language/VM substrate; explicit strict host UTF-8 input and fixed byte-output entry borrows now execute below the library, and a Wheeler module performs bounded nonnegative little-endian integer emission and checked eight-byte layout alignment for the bytecode-writer slice; Wheeler option/result, library `String`, normalization, generic maps/sets/queues, iteration, canonical collection encoding, decode errors, and packaged modules remain.
