...
- [ ] Wheeler bytecode codec and verifier: `SeedArtifact.w` emits a complete input-named minimal artifact with derived string and section layout through the same bounded encoding module; all 368 `LongClass` bytes match stage 0, decode, canonically re-encode, and execute. General IR-driven payloads and Wheeler verification remain.
