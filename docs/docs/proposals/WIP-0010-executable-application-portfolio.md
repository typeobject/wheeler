...
- [ ] Wheeler bytecode codec and verifier:
  - `compiler/Core.w` parses one linked bounded source and emits the full artifact through the shared encoding module. `compiler/Driver.w` owns the public graph-aware API. `MinimalCompiler.w` is its thin executable wrapper, while `NativeCompilerIdentity.w` keeps output private and publishes only the verified artifact digest.
