...
`NativeManifest.w` imports `packages/Manifest.w` and the shared Wheeler scanner. It accepts explicit UTF-8, requires `package STRING version STRING profile STRING ;` followed by zero or one bounded `target example STRING root STRING ;` and zero or one `dependency runtime STRING version STRING ;` and zero or one `capability STRING path STRING ;`, and returns quote-free typed source ranges without allocating host strings. `Semver.w` accepts bounded three-part releases and exact, caret, or tilde constraints, rejects leading zeroes and components above `Long.MAX_VALUE`, and deliberately leaves prereleases to the next slice. The executable fixture records header lengths `11`, `5`, and `11` plus target lengths `3` and `9` and dependency lengths `9` and `6` plus capability lengths `7` and `9` for `demo.native`, `1.2.3`, `bootstrap-1`, `app`, `src/App.w`, `demo.base`, `^1.0.0`, `fixture`, and `test-data`, rewinds exactly, and rejects a substituted declaration keyword, a leading-zero release, or an overflowing component. Other target/dependency kinds, module/source lists, other capability kinds, escape, duplicate, and canonical-output handling remain stage-0 work. This is the first parser rivet, not the whole bridge.
