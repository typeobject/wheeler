...
Stage 0 resolves XDG config/data/cache/state paths, diagnoses and ignores relative overrides, and atomically manages an ordered canonical-YAML repository policy. The implicit policy contains the stable `local` trust domain at the XDG data repository. Publication defaults there; exact fetch walks enabled authoritative repositories in policy order without merging them, or selects one alias explicitly. Physical `--registry` transport remains available for sealed bootstrap fixtures. Unlocked dependency resolution uses configured order by default, accepts repeated `--repository` aliases as an explicit ordered subset, or consumes one sealed `--catalog`; no artifact cache exists yet. [WIP-0023](../proposals/WIP-0023-recipe-repositories-and-reproducible-builds.md#xdg-local-objects-and-reusable-artifacts) owns repository/build-input/PREV semantics; a pathname does not get promoted to provenance merely because it lives under `$HOME`.
