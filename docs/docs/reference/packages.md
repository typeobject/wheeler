...
`RepositorySnapshotSignature` is the detached schema-1 Ed25519 envelope. It records the stable repository identity, snapshot identity, SHA-256 identity of the canonical X.509 public key, algorithm, and canonical base64 signature. The signed message is `wheeler-repository-snapshot-signature-1`, a zero byte, the 32 repository-identity bytes, and the complete canonical snapshot bytes. Parsing is strict before verification; changing a coordinate, repository, key, signature byte, YAML spelling, or DER spelling fails closed. The codec is implemented and differentially tested. Repository policy does not pin keys or require envelopes yet, so this envelope proves possession by a named key but does not by itself authorize that key. Cryptography can establish who signed the rubbish; policy still has to decide whether that person gets commit access.
