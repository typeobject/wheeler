...
`compiler/closure/ArchiveSources.w` starts the counted production path. It validates the outer package digest, every entry digest, 512 canonical sorted paths, complete framing, and exact path/data offsets before publishing a column. A 513th entry fails before hashing or mutation. Core ranged SHA-256 covers every block in the 16 MiB physical evidence ceiling.
