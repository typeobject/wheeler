...
`byteview` is the immutable binary counterpart at an entry or ordinary parameter. It exposes only checked octet indexing and `bufferLength`; it performs no UTF-8 validation, permits every byte sequence including empty input, and cannot be written, dropped, returned, or embedded in an aggregate. An entry declares either `utf8` or `byteview` input, never both, followed by an optional mutable `bytes` output. The embedding API selects the input kind explicitly. Guessing from content would make `0xc0 0x80` a protocol decision, which is not a job for wishful Unicode.
