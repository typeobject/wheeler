...
`wheeler site -o <directory>` is the sole website command. It discovers the fixed repository roots, creates the semantic bundle in private staging, independently verifies its exact profile/path/digest closure, and renders the inert Markdown subset to static HTML/CSS under `wheeler.doc-site/1`. The profile has one stylesheet, generated navigation, no scripts, no themes or plugins, a restrictive content-security policy, bounded output, and one atomic publication. `publication-manifest.json` binds the bundle, renderer classes, and every site file. Existing output is rejected. Java doclet nodes, executable examples, proof references, and generated reference tables remain WIP-0019 work. A few checked edges still do not make a graph database, which is fortunate for everyone involved.
