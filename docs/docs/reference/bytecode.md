...
`MinimalCompiler.w` and its dedicated compiler IR, token, parser, and encoding modules exercise the compiler/writer contract without asking Java to hold the pen: Wheeler scans and parses a bounded minimal source file, extracts its class and global IR, and emits a complete artifact containing zero or one signed global and one entry, optionally with a signed local declaration before a second state operation containing zero, one, or two parsed assignment/checked `+=`/`-=`/`^=`/global-expectation statements before `HALT` with source-derived identifiers sorted into the canonical string table, aligned offsets, and final size. For `LongClass` with `state long value = 7` and that update, CI compares all 504 bytes with stage 0, decodes it through the strict reader, requires byte-identical re-encoding, and executes it through both direct VM and `wheeler run` publication paths. Before publication, `compiler/Verifier.w` independently reads the emitted bytes and checks the canonical header, ordered contiguous directory, fixed payload invariants, function/local/code lengths, manifest bounds, and trailing `HALT`; malformed zero-filled input is rejected in Wheeler tests. This is a deliberately narrow verifier for the accepted compiler profile, not the production verifier in a novelty hat. This proves a complete source-to-artifact path for a narrow grammar with optional-global and assignment/arithmetic-instruction IR with derived zero through four local slots and instruction-stream sizes, with signed decimal constants encoded as little-endian two's complement and assertions lowered to local-free `EXPECT_EQ`, and local declarations lowered to `LOCAL_CONST` plus `LOCAL_MOVE` with following statement local bases derived rather than guessed; general declarations and control flow are not yet IR-driven. The 360-byte no-global `Bare` artifact and alternate identifier orders are differentially tested; duplicate names fail before output publication, because canonicalization is not a suggestion box.
