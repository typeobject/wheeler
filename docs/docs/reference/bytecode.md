...
`SeedArtifact.w` exercises the writer contract without asking Java to hold the pen: Wheeler emits a complete minimal artifact containing one `HALT` entry from a bounded ASCII class-name input, deriving string length, aligned offsets, and final size. For `LongClass`, CI compares all 368 bytes with stage 0, decodes it through the strict reader, requires byte-identical re-encoding, and executes it through both direct VM and `wheeler run` publication paths. This proves a complete fixed writer path; it does not yet make section construction IR-driven.
