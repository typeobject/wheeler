...
The manifest records the program kind (`classical`, `quantum`, or `hybrid`) in addition to its name, entry function, history ceiling, and step ceiling. Newly compiled source defaults the history ceiling to 250,000 records and the step ceiling to 1,000,000 transitions; both remain encoded, verifier-bounded policy inputs rather than format constants.
...
`MinimalCompiler.w` and its dedicated compiler IR, token, parser, code-generation, and encoding modules exercise the compiler/writer contract without asking Java to hold the pen: Wheeler scans and parses a bounded minimal source file, extracts its class and global IR, and emits a complete artifact containing zero or one signed global, an optional classical or reversible one- through four-statement helper, and one entry, containing zero through four parsed signed-local/assignment/checked `+=`/`-=`/`^=`/global-expectation statements before `HALT` with source-derived identifiers sorted into the canonical string table, aligned offsets, and final size. For `LongClass` with `state long value = 7` and that update, CI compares all 504 bytes with stage 0, decodes it through the strict reader, requires byte-identical re-encoding, and executes it through both direct VM and `wheeler run` publication paths. Before publication, `compiler/Verifier.w` independently reads the emitted bytes and checks the canonical header, ordered contiguous directory, fixed payload invariants, function/local/code lengths, manifest bounds, global and local type windows, every accepted instruction opcode/operand-count/encoded-length boundary, global/local/call operand domains, generated-inverse proof subjects and arguments, and terminal-only `HALT`; a Wheeler hex-input corpus accepts canonical stage-0 artifacts and rejects forged local/global indices, type codes, call targets, proof subjects, and proof arguments. `NativeVerifier.w` takes the same verifier over immutable binary `byteview`; direct and package-selected CLI tests feed it exact `.wbc` bytes, so verification no longer requires a text envelope. This is a deliberately narrow verifier for the accepted compiler profile, not the production verifier in a novelty hat. This proves a complete source-to-artifact path for a narrow grammar with optional-global and assignment/arithmetic-instruction IR with derived zero through eight local slots and instruction-stream sizes, with signed decimal constants encoded as little-endian two's complement and assertions lowered to local-free `EXPECT_EQ`, a helper lowered to two descriptors plus `RETURN` and one or two repeated `CALL` sites, entry statements lowered before and after a reverse block with separately derived local/type windows and forward lengths, reversible helpers lowering checked global add/subtract to opposite intrinsic bodies and XOR to identical self-inverse bodies in reversed statement order plus entry `CALL`/`UNCALL` and an optional post-inverse assertion, and optional inverse theorems lowered to a 28-byte `GENERATED_INVERSE` proof section, and local declarations lowered to `LOCAL_CONST` plus `LOCAL_MOVE` with following statement local bases derived rather than guessed; general declarations and control flow are not yet IR-driven. The 568-byte two-function `Calls` artifact, the 528-byte inverse-bearing `ReversibleCalls` artifact, post-call local/assertion forms, the proof-bearing `Certified` artifact, and the checked-in commented `Counter.w` source through direct and package-selected CLI compiler paths, the 360-byte no-global `Bare` artifact and alternate identifier orders are differentially tested; plain assignment in a `rev` helper and duplicate names fail before output publication, because canonicalization is not a suggestion box.

`compiler/Opcodes.w` is the Wheeler-side source authority for the opcode identities currently accepted by the bounded verifier/interpreter and for their shared family predicates. Its public `const long` declarations fold at compile time and add no globals or startup code. `Verifier.w` and `Interpreter.w` dispatch through those names rather than repeating integer ranges; stage 0's `Opcode` table remains the differential oracle until compiler promotion, not a license for a third table.
