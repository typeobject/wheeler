...
`MinimalCompiler.w` exercises the compiler/writer contract without asking Java to hold the pen: Wheeler scans and parses a bounded minimal source file, extracts its class and global IR, and emits a complete artifact containing one signed global and one entry that performs a parsed assignment or checked `+=`, `-=`, or `^=` update before `HALT` with source-derived identifiers sorted into the canonical string table, aligned offsets, and final size. For `LongClass` with `state long value = 7` and that update, CI compares all 504 bytes with stage 0, decodes it through the strict reader, requires byte-identical re-encoding, and executes it through both direct VM and `wheeler run` publication paths. This proves a complete source-to-artifact path for a narrow grammar with global and assignment/arithmetic-instruction IR with derived local counts and code sizes; general declarations and control flow are not yet IR-driven. Alternate identifier orders are differentially tested; duplicate names fail before output publication, because canonicalization is not a suggestion box.
