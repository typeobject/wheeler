...
The bounded compiler core now lives in importable `compiler/Core.w`. `compiler/Graphs.w` owns one- through three-module graph linking. `compiler/GraphFour.w` owns four-module forms. `compiler/GraphFourBranches.w` isolates root-heavy forms. `compiler/GraphFourMixed.w` isolates mixed-depth forms. `compiler/Driver.w` preserves the public API. `MinimalCompiler.w` is only its executable package wrapper. `compileMinimal` returns the exact verified artifact and code bounds without changing externally visible output length. A canonical module header qualifies entry and helper strings while preserving theorem names exactly as stage 0 does. It accepts up to sixty-four sorted unique direct import declarations. Malformed, duplicate, unsorted, and excess imports fail before publication. The four bounded constant-import APIs link one through four direct modules. The pair, triple, and four-module APIs also link one exact two-edge, three-edge, or four-edge scalar-constant chain, one two-leaf or three-leaf fork, one two-edge or three-edge chain beside a direct module, one two-edge chain beside two direct modules, two independent two-edge chains, or one two-leaf fork beside a direct module into unqualified or canonical owner-qualified public root uses. Private constants may feed public exports. Leaf exports become private in their dependent and do not leak into the root. The graph compiler evaluates and substitutes that graph before ordinary lowering, so imported declarations add no runtime section. Any private name in the root fails before caller output changes, including a root-local collision until separate symbol tables land. Executable imported members, mismatched names, wider graphs, more than four root imports, and non-ASCII linked sources also fail closed. Colliding exports, five or more imported modules, unrelated qualifiers, and general multi-file linking remain outside the slice. `NativeCompilerIdentity.w` compiles into private 4,096-byte storage and hashes only the returned range. Its digest matches the stage-0 artifact, while malformed or oversized source publishes nothing. This is an artifact-producing recovery boundary, not yet a general-purpose mutable semantic editor.
