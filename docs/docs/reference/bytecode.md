...
A class may declare up to sixty-four literal scalar constants before its helper or entry. The native resolver validates the complete prefix, rejects duplicate names, and substitutes matching signed or Boolean values into locals, direct helper returns, and same-type one-argument helper calls. Public and private declarations use the same table. Reordering independent declarations leaves output bytes unchanged. Constants contribute no global, initializer, or runtime lookup. Expressions and imported constants remain outside this recovery slice until they gain native graph evaluation rather than a hopeful token trick.
