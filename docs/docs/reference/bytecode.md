...
The accepted grammar supports zero or one signed global, an optional classical or reversible helper with zero through thirty-two statements, and one entry. An entry without a helper may contain zero through thirty-two signed-local, Boolean-local, assertion, assignment, checked-update, or global-expectation statements before `HALT`. Helper-call entries retain the smaller explicit call/reverse shape. Statement starts live in one caller-owned bounded table shared by both body parsers; the thirty-third row is not an undocumented storage tier.
