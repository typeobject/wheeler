...
The manifest records the program kind (`classical`, `quantum`, or `hybrid`) in addition to its name, entry function, history ceiling, and step ceiling. Newly compiled source defaults both history and step ceilings to 1,000,000; both remain encoded, verifier-bounded policy inputs rather than format constants.
