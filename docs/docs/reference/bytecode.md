...
`MinimalCompiler.w` exercises the compiler/writer contract without asking Java to hold the pen: Wheeler scans and parses a bounded minimal source file, extracts its class name, and emits a complete artifact containing one `HALT` entry with derived strings, aligned offsets, and final size. For `LongClass`, CI compares all 368 bytes with stage 0, decodes it through the strict reader, requires byte-identical re-encoding, and executes it through both direct VM and `wheeler run` publication paths. This proves a complete source-to-artifact path for one grammar production; general declarations and instructions are not yet IR-driven.
