...
| [`SeedArtifact.w`](../../wheeler-examples/src/main/wheeler/SeedArtifact.w) + [`compiler/Encoding.w`](../../wheeler-examples/src/main/wheeler/compiler/Encoding.w) | Complete Wheeler-written canonical artifact emission for a fixed minimal program | a 512-byte capacity is trimmed to 360 bytes; all bytes match stage 0, decode, re-encode identically, and execute to `HALTED`; `finalCursor = 360` |
