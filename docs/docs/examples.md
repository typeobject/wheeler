...
| [`NativeManifest.w`](../../wheeler-examples/src/main/wheeler/NativeManifest.w) + [`packages/Manifest.w`](../../wheeler-examples/src/main/wheeler/packages/Manifest.w) | Wheeler-native bounded scanning and typed parsing of the mandatory `wheeler.package` header, one bounded target, one dependency, and one path capability, with checked package names, logical paths, release/constraint syntax, and token-driven canonical re-emission to explicit host output | `package "demo.native" version "1.2.3-rc.1" profile "boot\"strap"; target example "app" root "src/App.w"; dependency normal "demo.base" version "^1.0.0"; capability "fixture" path "test-data";` yields `nameStart = 9`, header lengths `11`, `10`, `11`, target lengths `3`, `9`, dependency lengths `9`, `6`, capability lengths `7`, `9`, canonical output despite surplus input whitespace, and exact rewind; a wrong declaration keyword, uppercase/malformed package name, leading-zero release or numeric prerelease, and overflowing release component, traversal path, and backslash path trap instead of inventing a package |
