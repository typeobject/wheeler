...
| [`HostInput.w`](../../wheeler-examples/src/main/wheeler/HostInput.w) | One explicit strict bounded runtime UTF-8 input borrow with no ambient file/network access | for UTF-8 `A¢`: `byteLength = 3`, `scalarCount = 2`, `firstScalar = 65` |
...
| [`Utf8Lexer.w`](../../wheeler-examples/src/main/wheeler/Utf8Lexer.w) | Explicit host UTF-8 source input scanned into region-owned identifier/number/punctuation/comment tokens with checked decimal parsing and a bounded assignment-production parser | `tokenCount = 5`, `numberStart = 2`, `commentStart = 6`, `numericValue = 123`, `parseError = 0`, `finalCursor = 10` |
