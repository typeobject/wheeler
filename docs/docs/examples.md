...
Covers: One binary frame containing a canonical little-endian `u32` module count, one through four length-prefixed imported sources, and the remaining root source. The fixture calls the matching bounded constant-import API and publishes only the verified artifact range.

Expected behavior: One through four signed and Boolean constant modules with private and forward dependencies substitute through unqualified and repeated canonical owner-qualified public uses without adding runtime state and match stage 0 byte for byte. The pair may be two direct imports or one leaf-to-dependent-to-root chain. Three inputs may be direct root imports, one leaf-to-middle-to-dependent-to-root chain, two leaves feeding one dependent, or one chain beside a direct root import. Four inputs may be direct root imports, one four-edge chain, three leaves feeding one dependent, one three-edge chain beside a direct root import, one two-edge chain beside two direct root imports, two independent two-edge chains, or one two-leaf fork beside a direct module. Input order does not affect the artifact. Imported constants also lower through generated reversible helper updates and their inverse bodies. A private name in the root, colliding exports, wider graphs, executable imported members, mismatched module names, five imported modules, non-ASCII source, malformed framing, and linked source above 16,384 bytes publish nothing. Four honest imports now fit. A source map still does not.
...
Expected behavior: Empty-import one-module, two-external one-module, and three-, five-, nine-, and seventeen-module rooted DAG closures reproduce stage 0. A cycle, unreachable module, duplicate path, sixty-fifth module or external, unsorted external, unbound import, mismatched root, uppercase digest, traversal path, or input beyond 32,768 bytes publishes nothing. The current physical compiler closure has fifty-eight modules, 236 imports, and 21,076 canonical bytes. The packaged executable reproduces its stage-0 identity in 18,653,760 transitions. Sixty-four is still deliberately smaller than the 10,000-module schema. Pretending otherwise would merely give the graph a fake moustache too.
