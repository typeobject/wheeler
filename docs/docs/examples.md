...
Expected behavior: A public signed and Boolean constant graph with forward dependencies substitutes through unqualified and repeated canonical owner-qualified uses without adding runtime state and matches stage 0 byte for byte. Private constants, transitive imports, executable imported members, mismatched module names, multiple root imports, non-ASCII source, malformed framing, and linked source above 16,384 bytes publish nothing. This is one honest linker edge, not yet a source map wearing epaulettes.
...
Expected behavior: Empty-import one-module, two-external one-module, and three-, five-, nine-, and seventeen-module rooted DAG closures reproduce stage 0. A cycle, unreachable module, duplicate path, sixty-fifth module or external, unsorted external, unbound import, mismatched root, uppercase digest, traversal path, or input beyond 32,768 bytes publishes nothing. The current physical compiler closure has fifty-one modules, 209 imports, and 18,559 canonical bytes. The packaged executable reproduces its stage-0 identity in 14,891,995 transitions. Sixty-four is still deliberately smaller than the 10,000-module schema. Pretending otherwise would merely give the graph a fake moustache too.
