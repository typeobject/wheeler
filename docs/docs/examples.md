...
- Canonical `module examples.seed;` input emits stage-0-identical qualified entry/helper strings and unqualified theorem names, including stateless helper classes. A header may contain up to sixty-four sorted unique direct imports. Empty through sixty-four-import boundaries match stage 0 when imported declarations do not enter the root artifact. Malformed, duplicate, unsorted, and excess imports publish nothing. Public, explicit-private, and unqualified helpers produce the same linked function bytes, while duplicate visibility fails before output. Separate bounded paths link one direct scalar-constant module, two direct modules, or one leaf-to-dependent-to-root chain into unqualified or canonical owner-qualified public root uses. Private constants may feed public exports. Leaf exports become private in the dependent and do not leak into the root. Any private name in the root publishes nothing, including a root-local collision until separate symbol tables land. Executable imported members, mismatched names, wider graphs, more than two root imports, and non-ASCII linked source also fail closed. Colliding exports, three or more imports, unrelated qualifiers, and general multi-file linking remain outside this path. The header has acquired one small linker. Nobody should order it a swivel chair yet.
- `verification = 1`.
...
Covers: One binary frame containing a canonical little-endian `u32` module count, one or two length-prefixed imported sources, and the remaining root source. The fixture calls the singular or pair constant-import API and publishes only the verified artifact range.

Expected behavior: One or two signed and Boolean constant modules with private and forward dependencies substitute through unqualified and repeated canonical owner-qualified public uses without adding runtime state and match stage 0 byte for byte. The pair may be two direct imports or one leaf-to-dependent-to-root chain. Leaf exports become private in the dependent. Pair input order does not affect the artifact. Imported constants also lower through generated reversible helper updates and their inverse bodies. A private name in the root, colliding exports, wider graphs, executable imported members, mismatched module names, three imported modules, non-ASCII source, malformed framing, and linked source above 16,384 bytes publish nothing. These are still two honest linker edges, not a source map wearing epaulettes.
...
Expected behavior: Empty-import one-module, two-external one-module, and three-, five-, nine-, and seventeen-module rooted DAG closures reproduce stage 0. A cycle, unreachable module, duplicate path, sixty-fifth module or external, unsorted external, unbound import, mismatched root, uppercase digest, traversal path, or input beyond 32,768 bytes publishes nothing. The current physical compiler closure has fifty-two modules, 220 imports, and 19,186 canonical bytes. The packaged executable reproduces its stage-0 identity in 15,723,934 transitions. Sixty-four is still deliberately smaller than the 10,000-module schema. Pretending otherwise would merely give the graph a fake moustache too.
...
Covers: A preserved signed-parameter return relation, the implicit caller-owned result
slot, dedicated source-fill, call, and return instructions, and one generated-inverse
certificate. Core conformance
also commits VM history between the forward and inverse call. The Wheeler-native
compiler emits the same preserved-source helper, adjacent slot locals, generated bodies,
and proof bytes as stage 0. The Wheeler-native interpreter executes forward and inverse
call forms and checks restored vacancy against the Java VM. The inverse checks
`Holding(7)` against the preserved source instead of asking the debugger what happened.

Expected result: `observed = 7`.
