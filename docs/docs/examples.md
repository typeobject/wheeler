...
Covers: One binary frame containing a canonical little-endian `u32` module count, one through six length-prefixed imported sources, and the remaining root source. The fixture calls the matching bounded constant-import API and publishes only the verified artifact range.

Expected behavior: One through six signed and Boolean constant modules with private and forward dependencies substitute through unqualified and repeated canonical owner-qualified public uses without adding runtime state and match stage 0 byte for byte. The fixture covers every rooted tree topology over one through four imported modules, including direct stars, chains, balanced and uneven forks, mixed-depth branches, and a shared-dependency diamond. It also covers all 120 input orders of five direct imports, a five-module chain, a four-leaf fork, a three-leaf fork beside a direct import, one chain edge beside three direct imports, a two-leaf fork beside two direct imports, two independent chains beside a direct import, a three-module chain beside two direct imports, a four-module chain beside a direct import, a nested two-leaf fork beside a direct import, two nested fork levels, and a shared diamond with a side leaf. It checks all 720 orders of the six-module direct star, full chain, and five-leaf fork. Closed graph plans select the five-module and six-module forms before linking. The six-module plan admits the star, chain, or five-leaf fork only after exact header-edge, rooted-reachability, and topology checks. Input order does not affect the artifact. Imported constants also lower through generated reversible helper updates and their inverse bodies. A private name in the root, colliding exports, other five-module graphs, executable imported members, mismatched module names, another or disconnected six-module graph, seven imported modules, non-ASCII source, malformed framing, and linked source above 16,384 bytes publish nothing. Six honest imports fit in a row, a chain, or one well-fed fork. The pile still needs a real graph planner.
...
Covers: One through ninety-six sorted local source modules, zero through sixty-four externals, 320 total imports, unique paths, complete binding, rooted reachability, cycle rejection, bounded names and paths, lowercase source identities, exact schema bytes, SHA-256 publication, and rewind.

Expected behavior: Empty-import one-module, two-external one-module, and three-, five-, nine-, and seventeen-module rooted DAG closures plus a sixty-five-module star and ninety-six-module chain reproduce stage 0. A sixty-six-module star exceeds the per-module import bound. A cycle, unreachable module, duplicate path, ninety-seventh local module, sixty-fifth external, unsorted external, unbound import, mismatched root, uppercase digest, traversal path, or input beyond 32,768 bytes publishes nothing. The current physical compiler closure has seventy-seven modules, 294 imports, and 27,473 canonical bytes. The packaged executable reproduces its stage-0 identity in 30,149,615 transitions. Ninety-six is still deliberately smaller than the 10,000-module schema. Pretending otherwise would merely give the graph a fake moustache too.
