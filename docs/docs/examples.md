...
Expected behavior: One through three signed and Boolean constant modules with private and forward dependencies substitute through unqualified and repeated canonical owner-qualified public uses without adding runtime state and match stage 0 byte for byte. The pair may be two direct imports or one leaf-to-dependent-to-root chain. Three inputs may be direct root imports or one leaf-to-middle-to-dependent-to-root chain. Input order does not affect the artifact. Imported constants also lower through generated reversible helper updates and their inverse bodies. A private name in the root, colliding exports, wider graphs, executable imported members, mismatched module names, four imported modules, non-ASCII source, malformed framing, and linked source above 16,384 bytes publish nothing. Three honest imports now fit. A source map still does not.
...
Expected behavior: Empty-import one-module, two-external one-module, and three-, five-, nine-, and seventeen-module rooted DAG closures reproduce stage 0. A cycle, unreachable module, duplicate path, sixty-fifth module or external, unsorted external, unbound import, mismatched root, uppercase digest, traversal path, or input beyond 32,768 bytes publishes nothing. The current physical compiler closure has fifty-three modules, 224 imports, and 19,564 canonical bytes. The packaged executable reproduces its stage-0 identity in 16,266,721 transitions. Sixty-four is still deliberately smaller than the 10,000-module schema. Pretending otherwise would merely give the graph a fake moustache too.
