...
| [`SeedArtifact.w`](../../wheeler-examples/src/main/wheeler/SeedArtifact.w) + [`compiler/Encoding.w`](../../wheeler-examples/src/main/wheeler/compiler/Encoding.w) | Complete Wheeler-written canonical artifact emission for a bounded input-selected minimal class | input `LongClass` drives string sizing and aligned layout; a 512-byte capacity is trimmed to 368 bytes and all bytes match stage 0, decode, re-encode identically, and execute to `HALTED`; `finalCursor = 368` |
