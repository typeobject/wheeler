...
| [`MinimalCompiler.w`](../../wheeler-examples/src/main/wheeler/MinimalCompiler.w) + [`compiler/Parser.w`](../../wheeler-examples/src/main/wheeler/compiler/Parser.w) + dedicated IR/token/scanner/encoding modules | Wheeler compilation of one bounded minimal source grammar to canonical `.wbc` | input `classical class LongClass { state long value = 7; entry void main() { value += 5; } }` drives global and instruction IR, canonical lexical string ordering, function descriptors, and layout; the 504-byte result passes Wheeler's header/directory/payload verifier, matches stage 0, and executes with `value = 12`; `verification = 1`; no-global empty and signed-local-declaration classes plus stateful empty/assignment/`+=`/`-=`/`^=`/assertion/helper-call, reversible-helper/reverse-block, and generated-inverse theorem forms are differential-tested across two-, three-, or four-string tables, zero or one globals, and zero through four local/assignment/update/assertion statements; bounded bodies derive up to eight locals and 424 code bytes while `assert global == constant` lowers directly to local-free `EXPECT_EQ`; a named helper plus static entry call emits two descriptors, `RETURN`, and `CALL`; one following entry statement derives its own locals, type window, descriptor length, and code after the call; the `rev` form emits intrinsic forward/inverse bodies and entry `CALL`/`UNCALL`, then may assert the restored state; an optional theorem adds a canonically sorted proof name and 28-byte `GENERATED_INVERSE` section accepted by the proof kernel; local declarations work in no-global and stateful multi-statement bodies and lower through `LOCAL_CONST`/`LOCAL_MOVE`; signed decimal initializers and operands are emitted in canonical two's-complement form, while an overlong negative magnitude fails before publication rather than borrowing a convenient constant from the shrubbery; `codeStart = 392`, `finalCursor = 504` |
...
| [`NativeManifest.w`](../../wheeler-examples/src/main/wheeler/NativeManifest.w) + [`packages/Manifest.w`](../../wheeler-examples/src/main/wheeler/packages/Manifest.w) | Wheeler-native bounded scanning and typed parsing of the mandatory `wheeler.package` header, one bounded target, one dependency, and one path capability, with checked package names, logical paths, release/constraint syntax, and token-driven canonical re-emission to explicit host output | `package "demo.native" version "1.2.3-rc.1" profile "boot\"strap"; target example "app" root "src/A\"pp.w"; dependency normal "demo.base" version "^1.0.0"; capability "fixture" path "test-data";` yields `nameStart = 9`, header lengths `11`, `10`, `11`, target lengths `3`, `11`, dependency lengths `9`, `6`, capability lengths `7`, `9`, canonical output despite surplus input whitespace, and exact rewind; a wrong declaration keyword, uppercase/malformed package name, leading-zero release or numeric prerelease, and overflowing release component, traversal path, and backslash path trap instead of inventing a package |
