...
The executable core already has signed and Boolean values, immutable records, tagged variants, fixed arrays, nonescaping slices, typed calls, bounded loops, function-local regions with owned word/byte buffers, immutable UTF-8 owners, signed maps, strict validation, and deterministic classical function-module linking, canonical bytecode, deterministic package formats, and exact offline dependency inputs. Borrowing and mutable slices, UTF-8 strings, generic deterministic collections, full nominal/package modules, general mutable cross-function ownership/borrowing, and streaming/output effects remain under construction; explicit bounded UTF-8 input and bounded byte-output entry effects with checked publish lengths already execute.
