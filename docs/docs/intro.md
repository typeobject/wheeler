...
The executable core already has signed and Boolean values, immutable records, tagged variants, fixed arrays, nonescaping slices, typed calls, bounded loops, function-local regions with owned word/byte buffers, immutable UTF-8 owners, signed maps, and strict validation, canonical bytecode, deterministic package formats, and exact offline dependency inputs. Borrowing and mutable slices, UTF-8 strings, deterministic collections, modules, cross-function ownership, and explicit source effects remain under construction.
...
- signed and Boolean values, immutable records, tagged variants, fixed arrays, nonescaping slices, typed calls, recursion, conditionals, bounded loops, and function-local bounded regions with affine mutable word/byte buffers, immutable UTF-8 owners, and signed maps;
