...
Its native identity run completes in 83,741,348 transitions. The native compiler
...
names, source-selector paths, strict order, root coverage, source-row coordinates, target-tail keys, dependency coordinates, capability coordinates, and collection ordering have retained owners. The executable subset contains 470 functions and 16,304 instructions. The complete compiler has not reached a fixed
point.
