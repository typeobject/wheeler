...
414-module compiler closure with 1,970 imports and a 190,597-byte module manifest.
Its native identity run completes in 80,416,582 transitions. The native compiler
...
evidence compares 137 selected artifacts with stage 0 and closes every relocation.
...
Quoted range coordinates, header scalar state, format preamble, header name, and
header release validation have retained owners. The executable subset contains
426 functions and 15,161 instructions. The complete compiler has not reached a fixed
point.
