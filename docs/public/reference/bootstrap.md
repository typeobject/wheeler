...
427-module compiler closure with 2,009 imports and a 195,945-byte module manifest.
Its native identity run completes in 83,146,794 transitions. The native compiler
...
evidence compares 150 selected artifacts with stage 0 and closes every relocation.
...
fields, complete target-row coordinates, target test policy, and optional module
names have retained owners. The executable subset contains 448 functions and 15,995 instructions. The complete compiler has not reached a fixed
point.
