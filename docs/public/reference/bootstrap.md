...
415-module compiler closure with 1,973 imports and a 191,002-byte module manifest.
Its native identity run completes in 80,464,120 transitions. The native compiler
...
evidence compares 138 selected artifacts with stage 0 and closes every relocation.
...
Quoted range coordinates, header scalar state, format preamble, header name,
header release, and header-tail validation have retained owners. The executable
subset contains 427 functions and 15,227 instructions. The complete compiler has not reached a fixed
point.
