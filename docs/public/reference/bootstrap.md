...
419-module compiler closure with 1,986 imports and a 192,705-byte module manifest.
Its native identity run completes in 81,309,996 transitions. The native compiler
...
evidence compares 142 selected artifacts with stage 0 and closes every relocation.
...
Quoted range coordinates, complete header composition, and dependency-row prefix,
name, and version validation have retained owners. The executable subset contains
431 functions and 15,525 instructions. The complete compiler has not reached a fixed
point.
