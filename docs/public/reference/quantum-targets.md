...
`QuantumIo.request` wraps one complete submission and wait in the common `IoScope` lifecycle. Construction performs no target work. Queued cancellation allocates no job, while started cancellation propagates to the exact job and records acknowledged partial cancellation or uncertainty. Successful results revalidate the submission identity before terminal publication. Hybrid persistence still uses `QuantumJob` directly for acknowledged-job recovery, so the provider job API remains an interim adapter rather than a second portable source lifecycle.
...
`DynamicStateVectorSimulator` is the bounded target-resident control oracle. `DynamicStateVectorTarget` exposes that oracle through the ordinary asynchronous `QuantumTarget` submission and acknowledged-job recovery contract. Both advertise mid-circuit measurement, reset, and classical conditions separately from the static target. Canonical quantum region records prepare a register, measure one qubit into a target-resident Boolean slot, reset one qubit, and apply one fixed gate when a prior slot matches. A measured qubit cannot be measured or used by another gate until reset. The target executes one complete semantic region per submission without a host split. A region whose first record prepares its own register can establish the `HybridRun` submission state directly. The workflow still requires one explicit final full-register measurement when host code needs an observation. `DynamicTeleportationFixture` uses two measured slots and conditional X and Z corrections to transfer either basis input through one three-qubit target call. `DynamicSyndromeFixture` also executes up to 1,024 parity-measurement rounds in one target call. The results carry bounded final basis, slot, syndrome, reset, and correction evidence. This is semantic conformance evidence, not a noise model or hardware-fidelity claim.
...
`RecoverableOptimizerCampaign` executes one to 64 iterations of one to 64 bound submissions. Each binding has an explicit angle, probability, or scalar kind and must equal the submission map. The campaign persists acknowledged member identities after batch submission, recovers each member without resubmission, and records queued, running, failed, cancelled, completed, or unknown provider state. Results must match both job and submission identities. An already applied submission identity cannot change the objective twice. The snapshot store is an atomic adapter boundary. The campaign does not infer physical durability from an in-memory store.

`CalibrationAwareCompiler` extracts one to 64 direct semantic gates and submits one content-identified bounded calibration request. The provider result must name the exact request, target descriptor, gate set, epoch, and sample count. The default policy rejects old and future results. A bounded policy may admit an explicitly older epoch. The resulting plan binds the artifact, circuit, descriptor, request, result, policy outcome, exact duration, and additive parts-per-trillion error upper bound. It is planning evidence, not pulse lowering or a hardware-fidelity guarantee.

`DelegatedComputationSession` implements one bounded masked NOT transcript under the named honest-but-curious single-provider model. Provider-visible requests contain a nonce-backed commitment and blinded bit but no client mask or opening nonce. Challenge binding, exact relation verification, unmasking, and one-shot transcript consumption are executable. The fixture does not turn target metadata, redaction, or a deterministic test mask into a malicious-provider, collusion, side-channel, transport, randomness-quality, or general privacy theorem.
