...
A one-case descriptor over one root and up to seven local imported sources may carry zero artifact bytes. An entry case name must be the selected target plus `::entry`. The runtime compiles the validated source set natively and feeds the exact committed artifact into the same profile-2 path as transported bytes. A target with one parameterless root test and up to seven local imports may instead submit zero artifact bytes as `<target>::<declaration>`. Native discovery lowers and compiles that declaration without a Java-supplied artifact.
