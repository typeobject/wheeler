...
summary counts, complete execution and coverage identities, semantic report identities for up to 64 cases, and one- or two-case passing and assertion-failing classical artifact-to-report runs with transported descriptors, native source and case identities, pre-execution shard selection, canonical summary publication, runtime-owned failure diagnostics, and no Java semantic callback. Case identity, shard assignment, and summary reduction are canonical
