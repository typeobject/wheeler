...
Public reference pages describe implemented contracts. The [proposal guide](internal/proposals/index.mdx) groups internal WIPs by topic. Its [open-work map](internal/proposals/roadmap.md) lists unfinished contracts, and the [self-hosting status](internal/proposals/self-hosting-status.md) keeps compiler progress and evidence in one place. Internal WIPs contain designs, migrations, and milestone records. Future sketches remain internal until executable behavior belongs in the reference manual. Draft prose cannot make absent behavior callable.
