...
GitHub Pages runs that same command. Pull requests build the complete public site. Pushes publish the exact resulting directory. Internal documentation is checked for style and links by repository verification, but it has no public route, search node, navigation entry, or sitemap URL. There is no second website parser waiting in the shrubbery.
